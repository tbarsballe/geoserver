...
public interface Resource {
  ...
  public enum Type {
    ...
    DIRECTORY,
    /**
     * Resource used for content. Content access available through {@link Resource#in()} and {@link
     * Resource#out()}.
     */
    RESOURCE,
   ...
   *
   * <p>The resource may need to be unpacked into the GeoServer data directory prior to use. Do not
   * assume the file exists before calling this method.
   *
  ...
  /**
   * Path based resource access which can be used to access {@link #list()} contents or create a new
   * undefined resource.
   *
  ...
  /**
   * Writes a resource contents as a byte array. Usage is suggested only if the resource is known to
   * be small (e.g. a configuration file).
   *
   * @param byteArray
   * @throws IOException
   */
