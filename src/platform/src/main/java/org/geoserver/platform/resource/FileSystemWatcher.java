...
public class FileSystemWatcher implements ResourceNotificationDispatcher, DisposableBean {
  ...
  static class Delta {
    ...
    public Delta(
        File context, Kind kind, List<File> created, List<File> removed, List<File> modified) {
      this.context = context;
  ...
  /**
   * Note we have a single runnable here to review all outstanding Watch instances. The focus is on
   * using minimal system resources while we wait for Java 7 WatchService (to be more efficient).
   */
          ...
          for (Watch watch : watchers) {
            ...
            if (delta != null) {
              ...
              ResourceNotification notify =
                  new ResourceNotification(watch.getPath(), delta.kind, watch.last, events);
