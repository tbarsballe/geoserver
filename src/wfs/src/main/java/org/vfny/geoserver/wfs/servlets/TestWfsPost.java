  ...
  protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    if ((urlString == null)) {
      ...
      try {
        ...
        if (!doGet) {
          xmlOut =
              new PrintWriter(new BufferedWriter(new OutputStreamWriter(acon.getOutputStream())));
        ...
        if (acon.getResponseCode() >= 400) {
          // Construct the full response before writing, so that we don't throw an exception partway
          // through.
