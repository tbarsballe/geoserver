...
public abstract class NameSpaceElement {
   ...
   *
   * <p>The prefix is to be used for the qualification routines. If the prefix passed is null, then
   * qualified names will be null unless they have a prefix specified.
   *
   ...
   * <p>Creates an instance of this NameSpaceElement.
   *
   * <p>The prefix is to be used for the qualification routines is set to null. the qualified names
   * of the elements will be null
   */
   ...
   * getTypeDefName purpose.
   *
   * <p>This will return the name of the element. This method is useful when defining a new element
   * and wish to extend an existing element, such as <code>xs:string</code>.
   *
   * <p><code>
   * <xs:element name="id" type="xs:string"/>
   * <xs:element ref="gml:lineStringProperty" minOccurs="0"/>
   ...
   * for each Java class. This should be implemented the other way around, with a nice lookup table
   * to get the one and only default. But this is far easier to implement, as we just add this
   * method set to true for the namespace element classes we like best. If for some reason we set
   * two NSE's that map to the same java class to true then things will behave randomly, which is
   * why this is a bit of a hack. Apologies, it's late, and I need to finish my docs.
   */
