...
public class GetFeature {
  ...
  public FeatureCollectionResponse run(GetFeatureRequest request) throws WFSException {
    ...
    // be returned: "A value of SOME indicates that the WFS shall lock as many feature in the result
    // set as possible.
        ...
        try {
          ...
          for (int j = 0; j < metas.size(); j++) {
            ...
            if (!propertyNames.isEmpty()) {
              ...
              // only do this for simple features, complex mandatory features are handled by
              // app-schema
              if (meta.getFeatureType() instanceof SimpleFeatureType) {
                metaAllPropNames =
    ...
    return buildResults(
        request, totalOffset, maxFeatures, count, totalCount, results, lockId, getFeatureById);
  }
  ...
  /**
   * Expands the stored queries, returns true if a single GetFeatureById stored query was found (as
   * a different GML encoding is required in that case)
   *
   * @param request
   * @return
   */
  ...
  protected FeatureCollectionResponse buildResults(
    ...
    if (offset > 0 || count < Integer.MAX_VALUE) {
      ...
      // WFS 2.0 has specific requirements for hits, there is no previous link, the next points to
      // the first
      // page of results
  ...
  KvpMap buildKvpFromRequest(GetFeatureRequest request) {
    ...
    kvp.put(
        "RESULTTYPE",
        request.isResultTypeHits() ? ResultTypeType.HITS.name() : ResultTypeType.RESULTS.name());
    ...
    StringBuilder filter =
        q.getFilter() != null && q.getFilter() != Filter.INCLUDE ? new StringBuilder() : null;
  ...
  /**
   * Allows subclasses to poke with the feature collection extraction. The default behavior attempts
   * to wrap the feature collectio into a {@link FeatureSizeFeatureCollection}.
   *
   * @param source
   * @param gtQuery
   * @throws IOException
                ...
                if (filter.getSRS() != null
                  ...
                  GeographicBoundingBox valid =
                      (GeographicBoundingBox)
                          crs.getDomainOfValidity().getGeographicElements().iterator().next();
          ...
          new AbstractFilterVisitor() {
            ...
            private void checkNonSpatial(PropertyName pn) {
              ...
              if (ad instanceof GeometryDescriptor || isGmlBoundedBy(pn)) {
                throw new WFSException(
                    request,
                    "Cannot use a spatial property in a alphanumeric binary " + "comparison");
              }
            }
          };
