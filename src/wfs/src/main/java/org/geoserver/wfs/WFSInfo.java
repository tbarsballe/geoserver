...
public interface WFSInfo extends ServiceInfo {
    ...
    BASIC {
      ...
      public List<Operation> getOps() {
        return Arrays.asList(
            Operation.GETCAPABILITIES, Operation.DESCRIBEFEATURETYPE, Operation.GETFEATURE);
      }
    },
    TRANSACTIONAL {
      public int getCode() {
  ...
  /**
   * The flag which determines if gml:bounds elements should be encoded at the feature level in gml
   * output.
   */
  boolean isFeatureBounding();

  /**
   * Sets the flag which determines if gml:bounds elements should be encoded at the feature level in
   * gml output.
   */
