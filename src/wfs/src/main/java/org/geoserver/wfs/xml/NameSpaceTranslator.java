...
public abstract class NameSpaceTranslator {
   ...
   *
   * @param prefix The prefix for which this tranlator will tranlate. A null prefix will affect the
   ...
   * doesn't not depend on a real namespace map, but on careful assignment of the NamespaceElements,
   * so that each class only has one that returns true for isDefault(). Sorry for the hackiness, I
