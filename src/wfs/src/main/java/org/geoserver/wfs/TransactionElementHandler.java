...
public interface TransactionElementHandler {
     ...
     * @param featureTypeInfos a map from {@link QName} to {@link FeatureTypeInfo}, where
     ...
     * @param featureStores map from {@link QName} to {@link FeatureStore}, where the
     *                      keys do contain all the feature type names reported by
     *                      {@link #getTypeNames(EObject)}
     * @param response      the transaction response, that the element will update
     *                      according to the processing done
     * @param listener      a transaction listener that will be called before and after
