...
public class ShapeZipOutputFormat extends WFSGetFeatureOutputFormat
   ...
   *       is. The request may have specified the output file name through the {@code FILENAME }
   ...
   * SHAPE-ZIP_DEFAULT_PRJ_IS_ESRI property in metadata component of wfs.xml is true and there is an
   * entrance for EPSG code in user_projections/esri.properties file, then the .prj file is replaced
   * with a new one in ESRI WKT format. The content of the new file is extracted from
  ...
  private void replaceOGCPrjFileByESRIPrjFile(
    ...
    final Integer epsgCode =
        CRS.lookupEpsgCode(
            remappedSchema.getGeometryDescriptor().getCoordinateReferenceSystem(), true);
    if (epsgCode == null) {
