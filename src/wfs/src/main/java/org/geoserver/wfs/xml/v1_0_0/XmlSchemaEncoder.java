...
public class XmlSchemaEncoder extends WFSDescribeFeatureTypeOutputFormat {
  ...
  private final String generateTypes(FeatureTypeInfo[] infos, DescribeFeatureTypeType request)
    ...
    if (allSameType(infos)) {
      ...
      while (prefixIter.hasNext()) {
        ...
        tempResponse.append(
            getNSImport(prefix, infos, request.getBaseUrl(), request.getService().toLowerCase()));
      }
    }
  ...
  /**
   * Creates a import namespace element, for cases when requests contain multiple namespaces, as you
   * can not have more than one target namespace. See wfs spec. 8.3.1. All the typeNames that have
   * the correct prefix are added to the import statement.
   *
   * @param prefix the namespace prefix, which must be mapped in the main ConfigInfo, for this
   *     import statement.
   * @param typeNames a list of all requested typeNames, only those that match the prefix will be a
   ...
   * @task REVISIT: We need a way to make sure the extension bases are correct. should likely add a
   *     field to the info.xml in the featureTypes folder, that optionally references an extension
   *     base (should it be same namespace? we could also probably just do an import on the
   *     extension base). This function then would see if the typeInfo has an extension base, and
   *     would add or import the file appropriately, and put the correct substitution group in this
   *     function.
   */
  ...
  public String writeFile(File inputFile) throws IOException {
    ...
    try {
      ...
      throw (IOException)
          new IOException("problem writing featureType information " + " from " + inputFile)
              .initCause(e);
    }
