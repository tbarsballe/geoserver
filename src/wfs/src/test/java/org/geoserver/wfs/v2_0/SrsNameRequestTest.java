...
public class SrsNameRequestTest extends WFS20TestSupport {
  ...
  /**
   * Test that, for a given request srsName and bbox, the expected number of features are returned,
   * the response contains the expected feature PrimitiveGeoFeature.f015, and the pointProperty of
   * PrimitiveGeoFeature.f015 in the response has the expected srsName and data axis order
   ...
   * @param expectedData the expected pointProperty data of PrimitiveGeoFeature.f015 in the response
   *     or null if no features are expected
   */
  ...
  /**
   * Encode srsName (or a five-parameter bbox) so that it can be used in a GET request. Safe to use
   * (and has no effect on) a four-parameter bbox.
   *
  ...
  /**
   * Test response numberReturned, feature identity, srsName, and axis order for a request with an
   * EPSG code srsName and a latitude/longitude (correct axis order) native four-parameter bounding
   * box.
   */
  @Test
  ...
  /**
   * Test response numberReturned, feature identity, srsName, and axis order for a request with an
   * OGC HTTP URL srsName and a longitude/latitude (correct axis order) five-parameter bounding box.
   */
  @Test
  ...
  public void testUrnExperimentalBboxWrongAxisOrder() throws Exception {
    runTest(
        URN_EXPERIMENTAL_SRSNAME, buildBbox(LON_LAT_BBOX, URN_EXPERIMENTAL_SRSNAME), 0, null, null);
  }
  ...
  /**
   * Test response numberReturned, feature identity, srsName, and axis order for a request with an
   * OGC HTTP URI srsName and a latitude/longitude (correct axis order) five-parameter bounding box.
   */
  @Test
  public void testHttpUriBbox() throws Exception {
    runTest(
        HTTP_URI_SRSNAME, buildBbox(LAT_LON_BBOX, HTTP_URI_SRSNAME), 1, URN_SRSNAME, LAT_LON_DATA);
  }
