...
public class GetFeatureHitsIgnoreMaxFeaturesTest extends WFSTestSupport {
  ...
  public void testHitsIgnoreMaxFeaturesEnabled() throws Exception {
    ...
    assertEquals(
        "15",
        xpath
            .getMatchingNodes("//wfs:FeatureCollection/@numberOfFeatures", doc)
            .item(0)
            .getNodeValue());
  }

  /**
   * Test that doing a GetFeature request for data instead of hits still respects max features with
   * the hitsIgnoreMaxFeatures flag active
   */
  @Test
  public void testGetFeatureRespectsMaxFeatures() throws Exception {
    ...
    Document doc =
        getAsDOM("wfs?request=GetFeature&typename=cdf:Fifteen" + "&version=1.1.0&service=wfs");
