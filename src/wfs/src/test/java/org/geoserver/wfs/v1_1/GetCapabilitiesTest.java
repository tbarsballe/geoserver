...
public class GetCapabilitiesTest extends WFSTestSupport {
  ...
  public void testNamespaceFilter() throws Exception {
    ...
    XpathEngine xpath = XMLUnit.newXpathEngine();
    assertTrue(
        xpath.getMatchingNodes("//wfs:FeatureType/wfs:Name[starts-with(., sf)]", doc).getLength()
            > 0);
    assertEquals(
        0,
        xpath
            .getMatchingNodes("//wfs:FeatureType/wfs:Name[not(starts-with(., sf))]", doc)
            .getLength());
  ...
  public void testTypeNameCount() throws Exception {
    ...
    assertEquals(
        enabledCount,
        xpath
            .getMatchingNodes("/wfs:WFS_Capabilities/wfs:FeatureTypeList/wfs:FeatureType", doc)
            .getLength());
  }
  ...
  public void testLayerQualified() throws Exception {
    // filter on an existing namespace
    Document doc =
        getAsDOM("sf/PrimitiveGeoFeature/wfs?service=WFS&version=1.1.0&request=getCapabilities");
    ...
    assertEquals(
        7,
        xpath
            .getMatchingNodes("//ows:Get[contains(@xlink:href,'sf/PrimitiveGeoFeature/wfs')]", doc)
            .getLength());
    assertEquals(
        7,
        xpath
            .getMatchingNodes("//ows:Post[contains(@xlink:href,'sf/PrimitiveGeoFeature/wfs')]", doc)
            .getLength());
  ...
  public void testMetadataLinks() throws Exception {
    ...
    assertEquals(
        2,
        xpath
            .getMatchingNodes("//wfs:FeatureType[wfs:Name='cgf:MPolygons']/wfs:MetadataURL", doc)
            .getLength());
    assertEquals(
        2,
        xpath
    ...
    assertEquals(
        ...
        xpath
            .getMatchingNodes(
                "//wfs:FeatureType[wfs:Name='cgf:MPolygons']/wfs:MetadataURL[@type='FGDC']", doc)
            .getLength());
    assertEquals(
        2,
        xpath
  ...
  public void testOtherSRS() throws Exception {
    ...
    wfs.getSRS()
        .add("EPSG:4326"); // this one corresponds to the native one, should not be generated
  ...
  public void testSections(
      ...
      throws Exception {
    Document dom =
        getAsDOM("wfs?service=WFS&version=1.1.0&request=GetCapabilities&sections=" + sections);
    // print(dom);
