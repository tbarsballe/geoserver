...
public abstract class AbstractStylePage extends GeoServerSecuredPage {
  ...
  List<Exception> validateSLD() {
    try {
      ...
      List<Exception> validationErrors =
          styleHandler().validate(input, null, getCatalog().getResourcePool().getEntityResolver());
      input = new ByteArrayInputStream(style.getBytes());
      StyledLayerDescriptor sld =
          styleHandler()
              .parse(input, null, null, getCatalog().getResourcePool().getEntityResolver());
  ...
  /**
   * Check for an original CSS version of the style created by the old CSS extension (pre-pluggable
   * styles). If a CSS style is found, recover it if the derived SLD has not subsequently been
   * manually edited.
   *
   * <p>The recovery is accomplished by updating the catalog to point to the original CSS file, and
   * changing the style's format to "css".
   *
