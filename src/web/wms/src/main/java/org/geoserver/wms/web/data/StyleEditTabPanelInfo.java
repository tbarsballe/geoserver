...
public class StyleEditTabPanelInfo extends ComponentInfo<StyleEditTabPanel> {
  ...
  /**
   * It may be that a tab contribution to the {@link AbstractStylePage} need to work on a different
   * model object that the page's layer and resource models (for example, because it edits and saves
   * related information not directly attached to the style); if such is the case, this method shall
   * return the model to be passed to the {@link StyleEditTabPanel} constructor.
   *
   * <p>This default implementation just returns {@code null} and assumes the {@link
   * StyleEditTabPanel} described by this tab panel info works against the {@link AbstractStylePage}
   * StyleInfo model. Subclasses may override as appropriate.
   *
   * @param model
   * @param isNew
