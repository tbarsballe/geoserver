...
public class WMSAdminPage extends BaseServiceAdminPage<WMSInfo> {
  ...
  protected void build(IModel info, Form form) {
    ...
    MapModel maxAllowedFrames =
        defaultedModel(metadataModel, WMS.MAX_ALLOWED_FRAMES, WMS.MAX_ALLOWED_FRAMES_DEFAULT);
    ...
    form.add(
        new DropDownChoice("kml.superoverlayMode", kmlSuperoverlayMode, KML_SUPEROVERLAY_MODES));

    form.add(
        new CheckBox(
            "kml.kmattr", defaultedModel(metadataModel, WMS.KML_KMLATTR, WMS.KML_KMLATTR_DEFAULT)));
    form.add(
        new CheckBox(
            "kml.kmlplacemark",
    ...
    form.add(
        getFeatureInfoMimeTypesComponent =
            new MimeTypesFormComponent(
                ...
                new PropertyModel<Boolean>(info, "getFeatureInfoMimeTypeCheckingEnabled")
                    .getObject()));
  ...
  private class InterpolationRenderer extends ChoiceRenderer {
    ...
    public Object getDisplayValue(Object object) {
      return new StringResourceModel(((WMSInterpolation) object).name(), WMSAdminPage.this, null)
          .getString();
    }
