...
public class AdminPrivilegesTest extends GeoServerWicketTestSupport {
  ...
  public void testStyleAllPage() throws Exception {
    ...
    for (Iterator<Item> it = view.getItems(); it.hasNext(); ) {
      String name =
          it.next().get("itemProperties:0:component:link:label").getDefaultModelObjectAsString();
  ...
  public void testStyleEditPageGlobal() throws Exception {
    ...
    assertFalse(
        tester.getComponentFromLastRenderedPage("styleForm:context:panel:name").isEnabled());
    assertFalse(
        tester.getComponentFromLastRenderedPage("styleForm:context:panel:workspace").isEnabled());
    assertFalse(
        tester.getComponentFromLastRenderedPage("styleForm:context:panel:copy").isEnabled());
