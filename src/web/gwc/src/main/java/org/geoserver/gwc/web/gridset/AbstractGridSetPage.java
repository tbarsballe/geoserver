...
abstract class AbstractGridSetPage extends GeoServerSecuredPage {
  ...
  /**
   * Name of page parameter that holds the name of an existing gridset to use as template to create
   * a new one
   */
  ...
  public AbstractGridSetPage(final PageParameters parameters) {
    ...
    form.add(
        description =
            new TextArea<String>("description", new PropertyModel<String>(model, "description")));
  ...
  void computeBounds() {
    ...
    if (coordSys == null) {
      bounds.error(
          new StringResourceModel("AbstractGridsetPage.computeBounds.crsNotSet").getString());
      return;
    }
    ...
    if (aov == null) {
      bounds.error(
          new StringResourceModel("AbstractGridsetPage.computeBounds.aovNotSet").getString());
    } else {
  ...
  private static class UniqueNameValidator implements IValidator<String> {
    ...
    /**
     * @param previousName the initial name of the gridset when the page loaded, may be {@code null}
     *     only in case we're creating a new gridset
     */
