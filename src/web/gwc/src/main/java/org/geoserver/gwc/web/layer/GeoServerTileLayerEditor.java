...
class GeoServerTileLayerEditor extends FormComponentPanel<GeoServerTileLayerInfo> {
  ...
  public GeoServerTileLayerEditor(
    ...
    // Add the enabled/disabled parameter depending on the doCreateTileLayer variable if not already
    // set
    if (undefined) {
    ...
    configs.add(
        blobStoreId =
            new DropDownChoice<String>(
                "blobStoreId", blobStoreModel, blobStoreChoices, blobStoreRenderer));
        ...
        new IValidator<GeoServerTileLayerInfo>() {
          ...
          public void validate(IValidatable<GeoServerTileLayerInfo> validatable) {
            ...
            if (createVal && enabledVal && !isBlobStoreEnabled(blobStoreIdVal)) {
              error(
                  new ParamResourceModel("enabledError", GeoServerTileLayerEditor.this)
                      .getString());
            }
          }
        });
    ...
    enableInMemoryCaching =
        new CheckBox("inMemoryCached", new PropertyModel<Boolean>(getModel(), "inMemoryCached"));
    ...
    if (store != null && store.getCache() != null) {
      enableInMemoryCaching.setEnabled(
          mediator.getConfig().isInnerCachingEnabled() && !store.getCache().isImmutable());
    }
          ...
          protected Component getContents(final String id) {
            ...
            return new Label(
                ...
                new ParamResourceModel(
                    "confirmTileLayerRemoval", GeoServerTileLayerEditor.this, usedQuotaStr));
          }
