  ...
  private class GridSubsetListValidator implements IValidator<Set<XMLGridSubset>> {
    ...
    private void error(
      ...
      if (params == null) {
        ...
        message =
            new ParamResourceModel(resourceKey, GridSubsetsEditor.this, (Object[]) params)
                .getObject();
      }
    ...
    grids =
        new ListView<XMLGridSubset>(
          ...
          protected void populateItem(final ListItem<XMLGridSubset> item) {
            ...
            if (!gridsetExists) {
              gridSetLabel.add(
                  new AttributeModifier(
                      "style", new Model<String>("color:red;text-decoration:line-through;")));
              getPage().warn("GridSet " + gridSubset.getGridSetName() + " does not exist");
            }
            item.add(gridSetLabel);
            if (null != gridsetDescription) {
              gridSetLabel.add(
                  new AttributeModifier("title", new Model<String>(gridsetDescription)));
            }
            ...
            removeLink =
                new ImageAjaxLink<Object>("removeLink", GWCIconFactory.DELETE_ICON) {
            ...
            removeLink.add(
                new AttributeModifier("title", new ResourceModel("GridSubsetsEditor.removeLink")));
            item.add(removeLink);
          }
        };
