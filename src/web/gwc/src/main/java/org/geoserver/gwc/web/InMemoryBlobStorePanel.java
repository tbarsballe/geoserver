...
public class InMemoryBlobStorePanel extends Panel {
  ...
  public InMemoryBlobStorePanel(String id, final IModel<GWCConfig> gwcConfigModel) {
    ...
    IModel<Map<String, CacheConfiguration>> cacheConfigurations =
        new PropertyModel<Map<String, CacheConfiguration>>(gwcConfigModel, "cacheConfigurations");
    ...
    // Container containing all the parameters related to cache configuration which can be seen only
    // if In Memory caching is enabled
    ...
    if (store != null) {
      ...
      choice =
          new DropDownChoice<String>(
              "caches", providerClass, new ArrayList<String>(cacheProviders.keySet()), renderer);
      choice.add(
          ...
          protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
            try {
              ...
              if (store != null) {
                ...
                double actualSize =
                    ((long) (100 * (stats.getActualSize() * 1.0d) / byteToMb)) / 100d;
  ...
  static class MinimumConcurrencyValidator implements IValidator<Integer> {
    ...
    public void validate(IValidatable<Integer> iv) {
      if (iv == null || iv.getValue() <= 0) {
        ValidationError error = new ValidationError();
        error.setMessage(
            new ParamResourceModel("BlobStorePanel.invalidConcurrency", null, "").getObject());
        iv.error(error);
      }
    }
  }
  ...
  static class CacheConfigContainerWrapper extends WebMarkupContainer {
    ...
    /**
     * This method removes all the previous mappings from the container and then adds the components
     * again by setting as default value the one taken from the key mapped.
     *
     * @param key
