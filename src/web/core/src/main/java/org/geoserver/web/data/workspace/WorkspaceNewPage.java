...
public class WorkspaceNewPage extends GeoServerSecuredPage {
  ...
  /**
   * Helper method that takes care of storing the user entered workspace information and associated
   * namespace. This method makes sure that or both the workspace and namespace are successfully
   * stored or none is stored.
   *
  ...
  /**
   * Helper method that checks in the case of an exception if both the workspace and namespace where
   * created or removed, if it is not the case then removes the remaining one. The invoker is
   * responsible to log the exception as needed.
   *
