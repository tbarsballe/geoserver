...
interface GeoServerNodeInfo {
  ...
  /**
   * The node id, displayed as a label in the GUI. If null is returned, the element will be hidden.
   */
  ...
  /**
   * Allows customization of the label container, in particular, its style and visibility, but also
   * any other attribute. Implementors are suggested to hide the id element unless an administrator
   * is logged in, but specific implementations can use different policies. See {@link
   * DefaultGeoServerNodeInfo} for the default implementation of the visibility logic
   *
   * @param parent
   */
