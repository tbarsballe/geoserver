...
public class SQLViewAttributeProvider extends GeoServerDataProvider<SQLViewAttribute> {
  ...
  static final Property<SQLViewAttribute> NAME = new BeanProperty<SQLViewAttribute>("name", "name");
  ...
  static final Property<SQLViewAttribute> SRID = new BeanProperty<SQLViewAttribute>("srid", "srid");
      ...
      if (Geometry.class.isAssignableFrom(att.getType())) {
        if (att.getSrid() == null) {
          ...
          vt.addGeometryMetadatata(
              att.getName(), (Class<? extends Geometry>) att.getType(), att.getSrid());
        }
      }
      if (att.pk) {
        pks.add(att.getName());
