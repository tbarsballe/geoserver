...
public class ConfirmRemovalPanel extends Panel {
  ...
  public ConfirmRemovalPanel(String id, List<? extends CatalogInfo> roots) {
    ...
    modified.setVisible(
        visitor.getObjects(null, EXTRA_STYLE_REMOVED, GROUP_CHANGED, STYLE_RESET).size() > 0);
    add(modified);
   ...
   *
   * <p>This method returns non-null in cases where the object should not be be removed. The return
   * value should be a description or reason of why the object can not be removed.
   *
   * @param info The object to be removed.
   * @return A message stating why the object can not be removed, or null to indicate that it can be
   *     removed.
   */
