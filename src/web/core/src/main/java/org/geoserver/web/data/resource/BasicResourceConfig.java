...
public class BasicResourceConfig extends ResourceConfigurationPanel {
  ...
  public BasicResourceConfig(String id, IModel<ResourceInfo> model) {
    ...
    final EnvelopePanel latLonPanel =
        new EnvelopePanel(
            "latLonBoundingBox", new PropertyModel<ReferencedEnvelope>(model, "latLonBoundingBox"));
    ...
    CRSPanel nativeCRS =
        new CRSPanel("nativeSRS", new PropertyModel<CoordinateReferenceSystem>(model, "nativeCRS"));
    nativeCRS.setReadOnly(true);
    refForm.add(nativeCRS);
    declaredCRS =
        new CRSPanel("declaredSRS", new SRSToCRSModel(new PropertyModel<String>(model, "sRS")));
  ...
  /**
   * Form validator that checks whether the native CRS can be projected to the declared one whenever
   * the projection policy chosen is "reproject"
   */
