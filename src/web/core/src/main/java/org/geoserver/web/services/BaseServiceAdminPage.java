...
public abstract class BaseServiceAdminPage<T extends ServiceInfo> extends GeoServerSecuredPage {
  ...
  void init(final IModel<T> infoModel) {
    ...
    form.add(
        new Label(
            "service.enabled",
            new StringResourceModel("service.enabled", this).setParameters(getServiceName())));
    ...
    form.add(
        new KeywordsEditor(
            "keywords", LiveCollectionModel.list(new PropertyModel(infoModel, "keywords"))));
   ...
   *
   * <p>The form uses a {@link CompoundPropertyModel} so in the normal case components do not need a
   * model as its inherited from the parent. This means that component id's should match the info
   * bean property they correspond to.
   *
