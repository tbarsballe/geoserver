...
public class EnvelopePanel extends FormComponentPanel<ReferencedEnvelope> {
  ...
  /**
   * Makes the CRS bounds a required component of the envelope. It is warmly suggested that the crs
   * field be made visible too
   *
   * @param crsRequired
   */
    ...
    if (minX != null && maxX != null && minY != null && maxY != null) {
      if (crsRequired && crs == null) {
        ...
        if (is3D()) {
          ...
          setConvertedInput(
              new ReferencedEnvelope3D(minX, maxX, minY, maxY, minZsafe, maxZsafe, crs));
        } else {
