...
public class GeoServerLoginPage extends GeoServerBasePage {
  ...
  public GeoServerLoginPage(PageParameters parameters) {
    ...
    HttpSession session =
        ((HttpServletRequest) ((WebRequest) getRequest()).getContainerRequest()).getSession();

    // TODO: (from the spring security sources):  @deprecated If you want to retain the username,
    // cache it in a customized {@code AuthenticationFailureHandler}
    ...
    try {
      if (parameters.get("error").toBoolean()) {
        ...
        if (exception instanceof ConcurrentAuthenticationException) {
          ...
          error(
              new ParamResourceModel("concurrentAuthenticationError", this, cae.getCount())
                  .getString());
        } else {
