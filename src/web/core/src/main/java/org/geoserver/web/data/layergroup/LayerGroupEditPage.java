...
public class LayerGroupEditPage extends PublishedConfigurationPage<LayerGroupInfo> {
  ...
  public class LayerGroupTab extends PublishedEditTabPanel<LayerGroupInfo> {
    ...
    private void initUI() {
      ...
      queryableCheckBox =
          new CheckBox("queryable", new Model<Boolean>(!getPublishedInfo().isQueryDisabled()));
      ...
      add(
          lgEntryPanel =
              new LayerGroupEntryPanel("layers", getPublishedInfo(), wsChoice.getModel()));
      ...
      add(
          new KeywordsEditor(
              "keywords",
              LiveCollectionModel.list(new PropertyModel<List<KeywordInfo>>(myModel, "keywords"))));
