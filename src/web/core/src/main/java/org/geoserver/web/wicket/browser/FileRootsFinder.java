...
public class FileRootsFinder implements Serializable {
  ...
  /**
   * Utility so split and rebuild paths accounting for ResourceStore little own illusion of working
   * on a *nix file system regardless of the actual file system
   */
   ...
   * an a {@link Stream} containing the actual paths matching the provided input (any file/directory
   * starting with the same path as the input and containing the file name in a case insensitive
   * way)
   *
   * @param input A partial path, can be a single name or a full path (can be relative, will be
   *     matched against the data directory)
   * @param fileFilter An optional file filter to filter the returned files. The file filter should
   *     accept directories.
   * @return
   */
  public Stream<String> getMatches(String input, FileFilter fileFilter) {
    ...
    Stream<String> result =
        resource
            ...
            .filter(
                r -> fileFilter == null || fileFilter.accept(new File(dataDirectoryRoot, r.path())))
        ...
        Stream<String> rootPaths =
            Arrays.stream(names)
                .filter(
                    fileName ->
                        fileFilter == null
                            || fileFilter.accept(new File(fsSplitter.base, fileName)))
