...
public class WorkspaceEditPage extends GeoServerSecuredPage {
  ...
  private void init(WorkspaceInfo ws) {
    ...
    if (ns == null) {
      // unfortunately this may happen if the namespace associated to the workspace was deleted or
      // never created
    ...
    TextField<String> uri =
        new TextField<String>("uri", new PropertyModel<String>(nsModel, "uRI"), String.class);
  ...
  class SettingsPanel extends FormComponentPanel<Serializable> {
    ...
    public SettingsPanel(String id, IModel<WorkspaceInfo> model) {
      ...
      set.model =
          settings != null ? new ExistingSettingsModel(wsModel) : new NewSettingsModel(wsModel);
      ...
      ListView<Service> serviceList =
          new ListView<Service>("services", services) {
            ...
            protected void populateItem(ListItem<Service> item) {
              ...
              image.add(
                  new AttributeModifier("alt", new ParamResourceModel(info.getTitleKey(), null)));
              link.add(image);
              item.add(link);
            }
          };
