...
public class PreviewLayer {
  ...
  GetMapRequest getRequest() {
    if (request == null) {
      ...
      try {
        ...
        LOGGER.log(
            Level.INFO,
            "Could not set figure out automatically a good preview link for " + getName(),
            e);
      }
    }
    return request;
   ...
   * finds AbstractFeatureType. Then, the GML version is determined by looking at the namespace URI.
   *
   * <p>Please note that this method does not differentiate between GML 2 and GML 3.1.1, but assumes
   * that "http://www.opengis.net/gml" namespace always refers to GML 3.1.1.
   *
