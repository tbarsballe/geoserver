...
public class KMLSuperOverlayTest extends WMSTestSupport {
  ...
  protected void onSetUp(SystemTestData testData) throws Exception {
    ...
    testData.addVectorLayer(
        DISPERSED_FEATURES, Collections.EMPTY_MAP, "Dispersed.properties", getClass(), catalog);
  ...
  public void testCrossingSuperoverlay() throws Exception {
    ...
    assertEquals(
        -105.2243,
        Double.parseDouble(
            xpath.evaluate("//kml:Document/kml:Folder/kml:LookAt/kml:longitude", document)),
        1E-4);
    assertEquals(
        40.0081,
        Double.parseDouble(
            xpath.evaluate("//kml:Document/kml:Folder/kml:LookAt/kml:latitude", document)),
        1E-4);
  }
  ...
  public void testSuperOverlayLinkContents() throws Exception {
    ...
    Document document =
        getAsDOM("wms/kml?layers=" + getLayerId(MockData.BASIC_POLYGONS) + "&mode=superoverlay");
    // print(document);
  ...
  /**
   * Verify that when a tile smaller than one hemisphere is requested, then subtiles are included in
   * the result (but only the ones necessary for the data at hand)
   */
  @Test
  ...
  public void testGWCIntegration() throws Exception {
    ...
    assertEquals(
        "never", xpath.evaluate("//kml:NetworkLink/kml:Link/kml:viewRefreshMode", document));
  }
