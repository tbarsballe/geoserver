...
public class KMLReflectorTest extends WMSTestSupport {
  ...
  public void testDownloadMultiLayer() throws Exception {
    String requestURL =
        "wms/kml?&layers=" + getLayerId(MockData.LAKES) + "," + getLayerId(MockData.FORESTS);
  ...
  public void testForceRasterKml() throws Exception {
    ...
    String href =
        XMLUnit.newXpathEngine().evaluate("//kml:Folder/kml:GroundOverlay/kml:Icon/kml:href", dom);
  ...
  public void testForceRasterKmz() throws Exception {
    ...
    assertEquals(
        "attachment; filename=cite-BasicPolygons.kmz", response.getHeader("Content-Disposition"));
  ...
  public void testRelativeDynamicSymbolizer() throws Exception {
    final String requestUrl =
        "wms/kml?layers=" + getLayerId(MockData.STREAMS) + "&styles=relativeds&mode=download";
    Document document = getAsDOM(requestUrl);
        ...
        SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI)
            .newSchema(
                new Source[] {
                  ...
                  new StreamSource(
                      getClass().getResource("/schema/ogckml/ogckml22.xsd").toExternalForm())
                }));
  ...
  /**
   * Method testLookatOptionsWithRefreshMode tests if the two altitude values are obtained from the
   * corresponding bounding box. The first value (//kml:Document/kml:LookAt/kml:altitude) is
  ...
  /**
   * Method testWMSTimeRequest tests if the time parameter of the request is also passed to the KML
   * WMS request.
   *
