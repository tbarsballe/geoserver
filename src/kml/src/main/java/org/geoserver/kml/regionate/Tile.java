...
public class Tile {
   ...
   *
   * <ul>
   *   <li>centroids sitting on the tile borders must be associated to exactly one tile, so we have
   *       to consider only two borders as inclusive in general (S and W) but add on occasion the
   *       other two when we reach the extent of our data set
  ...
  public Tile(ReferencedEnvelope wgs84Envelope) {
    ...
    x =
        Math.round(
            ((wgs84Envelope.getMinimum(0) - WORLD_BOUNDS.getMinimum(0)) / MAX_TILE_WIDTH)
                * Math.pow(2, z));
    y =
        Math.round(
            ((wgs84Envelope.getMinimum(1) - WORLD_BOUNDS.getMinimum(1)) / MAX_TILE_WIDTH)
                * Math.pow(2, z));
    envelope = envelope(x, y, z);
  }
