...
public class PlainFolderSequenceFactory extends AbstractFolderSequenceFactory {
  ...
  public class PlainFolderGenerator extends AbstractFolderGenerator {
      ...
      if (layer instanceof FeatureLayer) {
        ...
        if (useVectorOutput(context)) {
          List<Feature> features =
              new SequenceList<Feature>(new FeatureSequenceFactory(context, (FeatureLayer) layer));
    ...
    /**
     * Adds the feature centroids to the output features, without actually adding the full geometry
     * (used when doing raster overlays of vector data with a desire to retain the popups)
     *
     * @param layer
     * @param folder
     */
    ...
    /**
     * Determines whether to return a vector (KML) result of the data or to return an image instead.
     * If the kmscore is 100, then the output should always be vector. If the kmscore is 0, it
     * should always be raster. In between, the number of features is weighed against the kmscore
     * value. kmscore determines whether to return the features as vectors, or as one raster image.
     * It is the point, determined by the user, where X number of features is "too many" and the
     * result should be returned as an image instead.
     *
