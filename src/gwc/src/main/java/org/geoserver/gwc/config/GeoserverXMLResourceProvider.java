...
public class GeoserverXMLResourceProvider implements ConfigurationResourceProvider {
  ...
  /**
   * Helper method that infers the directory that contains or will contain GWC configuration. First
   * we will check if a specific location was set using properties GEOWEBCACHE_CONFIG_DIR and
   * GEOWEBCACHE_CACHE_DIR, then we will check if a location was provided and then fallback on the
   * default location.
   */
  private static Resource inferConfigDirectory(
      ResourceStore resourceStore, String providedConfigDirectory) {
    // check if a specific location was provided using a context property otherwise use the provided
    // directory
  ...
  private static Optional<String> findFirstDefined(String... propertiesNames) {
    for (String propertyName : propertiesNames) {
      ...
      if (propertyValue != null) {
        ...
        LOGGER.debug(
            String.format("Property '%s' is set with value '%s'.", propertyName, propertyValue));
