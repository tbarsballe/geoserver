...
public class ConfigurableQuotaStoreProvider extends QuotaStoreProvider {
  ...
  public ConfigurableQuotaStoreProvider(
    ...
    boolean disabled =
        Boolean.valueOf(GeoServerExtensions.getProperty(GWC_DISKQUOTA_DISABLED)).booleanValue();
    if (disabled) {
      LOGGER.warning(
      ...
      if (!(oldStore instanceof DummyQuotaStore)) {
        try {
          ...
          try {
            ...
            LOGGER.log(
                Level.SEVERE, "An error occurred while closing up the previous quota store", e);
          }
        }
      }
    }
