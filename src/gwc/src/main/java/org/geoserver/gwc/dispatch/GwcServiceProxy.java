...
public class GwcServiceProxy {
  ...
  private final Pattern kmlXyzPattern =
      Pattern.compile(".*x(?<x>[0-9]+)y(?<y>[0-9]+)z(?<z>[0-9]+).*?", Pattern.CASE_INSENSITIVE);
   ...
   * to find such a method and check if the returned service info is {@link ServiceInfo#isEnabled()
   * enabled} (and hence avoid the WARNING message it spits out if this method is not found); not
   * though, that in the interest of keeping a single {@link GwcServiceProxy} to proxy all gwc
   * provided services (wmts, tms, etc), the service info returned here will always be enabled, we
   * already have a GWC {@link GWCServiceEnablementInterceptor service interceptor} aspect that
   ...
   * HttpServletResponse) handleRequest(HttpServletRequest, HttpServletResponse)} method, and return
   * a response object so that the GeoServer {@link Dispatcher} looks up a {@link Response} that
   * finally writes the result down to the client response stream.
   *
   * @param rawRequest
   * @param rawRespose
