...
public class CatalogConfiguration implements TileLayerConfiguration {
  ...
  public GeoServerTileLayer getTileLayerById(final String layerId) {
    ...
    try {
      ...
      if (localWorkspace != null) {
        ...
        // if the layer matches the virtual service workspace we return the layer otherwise NULL is
        // returned
  ...
  public synchronized void addLayer(final TileLayer tl) {
    ...
    if (layerInfo != null && !isLayerExposable(layerInfo)) {
      LOGGER.warning(
          "Requested layer " + layerInfo.getName() + " has no geometry. Won't create TileLayer");
      return;
    }
    ...
    try {
      ...
      checkArgument(
          notExists, "A GeoServerTileLayer named '" + info.getName() + "' already exists");
  ...
  public synchronized void renameLayer(String oldName, String newName)
    ...
    TileLayer tl =
        getLayer(oldName)
            .orElseThrow(() -> new NullPointerException("TileLayer " + oldName + " not found"));
    ...
    try {
      ...
      if (tileLayerInfo != null) {
        ...
        try {
          ...
          LOGGER.log(
              Level.SEVERE,
              "Error deleting tile layer '" + tileLayerInfo.getName() + "' from cache",
              e);
        }
  ...
  /**
   * Sets the {@link GridSetBroker} for use by this configuration. Automatically called by spring on
