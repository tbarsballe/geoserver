...
public class GeoServerTileLayer extends TileLayer implements ProxyLayer {
  ...
  public String getName() {
    ...
    if (gwcOperation != null && gwcOperation.equalsIgnoreCase("GetCapabilities")) {
      // this is a get capabilities request, we need to check if we are in the context of virtual
      // service
    ...
    for (XMLGridSubset gridSubset : cachedGridSets) {
      ...
      if (null == extent) {
        try {
          ...
          LOGGER.log(
              Level.WARNING, "Error computing layer bounds, assuming whole GridSet bounds", e);
  ...
  private BoundingBox getBounds(final SRS srs) {
    ...
    try {
      ...
      LOGGER.log(
          Level.FINE,
          "Can't compute tile layer bouds out of resource " + "native bounds for CRS " + srs,
          e);
    ...
    for (MetadataLinkInfo gsMetadata : gsMetadataLinks) {
      ...
      try {
        gwcMetadataLinks.add(
            new MetadataURL(gsMetadata.getMetadataType(), gsMetadata.getType(), new URL(url)));
  ...
  public Map<String, org.geowebcache.config.legends.LegendInfo> getLayerLegendsInfo() {
    ...
    for (StyleInfo styleInfo : styles) {
      ...
      try {
        scalesDenominator =
            CapabilityUtil.searchMinMaxScaleDenominator(Collections.singleton(styleInfo));
      ...
      if (legendInfo != null) {
        ...
        gwcLegendInfo
            ...
            .withCompleteUrl(
                buildURL(
                    baseUrl, legendInfo.getOnlineResource(), null, URLMangler.URLType.RESOURCE));
   ...
   * HTTP request is in progress NULL is returned. Only the use cases where an OWS service or a REST
   * end-point was target are handled.
   */
