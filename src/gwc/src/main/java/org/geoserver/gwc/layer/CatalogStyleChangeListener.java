...
public class CatalogStyleChangeListener implements CatalogListener {
   ...
   * about the name of the style. For the tiled layer its different, because styles are referred to
   * by {@link GeoServerTileLayerInfoImpl#cachedStyles() name} in order to create the appropriate
   * "STYLES" {@link ParameterFilter parameter filter}. This method will look for any tile layer
   * backed by a {@link LayerInfo} that has a cache for the renamed style as an alternate style
   * (i.e. through a parameter filter) and will truncate the cache for that layer/style. This is so
   * because there's no way in GWC to just rename a style (that would imply getting to the parameter
   * filters that refer to that style in the meta-store and change it's value preserving the
   * parametersId)
   *
   * <p>
   *
  ...
  public void handleModifyEvent(CatalogModifyEvent event) throws CatalogException {
    ...
    if (source instanceof StyleInfo) {
      ...
      if (nameIdx != -1) {
        // for now, only handle the even if the name changes. most likely we should also do the
        // truncate if
  ...
  private void handleStyleRenamed(final String oldStyleName, final String newStyleName) {
    ...
    for (GeoServerTileLayer tl : affectedLayers) {
      ...
      if (styleNames.contains(oldStyleName)) {
        ...
        String defaultStyle =
            layerInfo.getDefaultStyle() == null ? null : layerInfo.getDefaultStyle().prefixedName();
  ...
  private void handleStyleChange(final StyleInfo modifiedStyle) {
    ...
    for (LayerInfo affectedLayer : layers) {
      ...
      log.info(
          "Truncating layer '" + prefixedName + "' due to a change in style '" + styleName + "'");
