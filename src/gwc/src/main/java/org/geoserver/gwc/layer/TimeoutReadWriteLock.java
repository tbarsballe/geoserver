...
class TimeoutReadWriteLock {
  ...
  /**
   * Acquires a read lock with the configured timeout, will throw a {@link ServiceException} if the
  ...
  /**
   * Acquires a write lock with the configured timeout, will throw a {@link ServiceException} if the
   * lock is not acquired
   */
