...
public class GWCIntegrationTest extends GeoServerSystemTestSupport {
  ...
  protected void onSetUp(SystemTestData testData) throws Exception {
    ...
    testData.addVectorLayer(
        BASIC_POLYGONS_NO_CRS, null, "BasicPolygonsNoCrs.properties", this.getClass(), catalog);
  }
  ...
  /**
   * If direct WMS integration is enabled, a GetMap requests that hits the regular WMS but matches a
   * gwc tile should return with the proper {@code geowebcache-tile-index} HTTP response header.
   */
  @Test
  ...
  public void testDirectWMSIntegrationWithVirtualServicesAndWorkspacedStyle() throws Exception {
    ...
    assertEquals(
        newWorkspaceName + ":" + WORKSPACED_LAYER, response4.getHeader("geowebcache-layer"));
  ...
  public void testBasicIntegration() throws Exception {
    ...
    for (TileLayer tl : tld.getLayerList()) {
      if (tl.getName().equals("sf:AggregateGeoFeature")) {
        ...
        assertTrue(
            mime.startsWith("image/") || mime.startsWith("application/vnd.google-earth.kml+xml"));
      }
    }
    assertTrue(foudAGF);
  ...
  public void testPreserveHeaders() throws Exception {
    ...
    assertEquals(
        "inline;filename=wms-getcapabilities.xml", response.getHeader("content-disposition"));
  }
  ...
  public void testGetCapabilitiesWithLocalGroup() throws Exception {
    ...
    assertThat(
        WMTS_XPATH_10.evaluate(
            "count(//wmts:Contents/wmts:Layer[ows:Identifier='" + SIMPLE_LAYER_GROUP + "'])",
            document),
        is("1"));
  }
  ...
  public void testWmtsGetCapabilitiesRequest() throws Exception {
    ...
    try {
      response =
          getAsServletResponse(TEST_WORKSPACE_NAME + "/gwc/service/wmts?request=GetCapabilities");
  ...
  /**
   * Test that using an invalid \ non-existing style in a GetTile request will throw the correct OWS
   * exception.
   */
  @Test
