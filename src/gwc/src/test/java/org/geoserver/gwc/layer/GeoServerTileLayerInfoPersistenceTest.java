...
public class GeoServerTileLayerInfoPersistenceTest {
  ...
  <T> Matcher<T> sameProperty(T expected, String property, Function<?, Matcher<?>> valueMatcher)
      throws Exception {
    Object value =
        Arrays.stream(Introspector.getBeanInfo(expected.getClass()).getPropertyDescriptors())
            .filter(p -> p.getName().equals(property))
            .findAny()
            .orElseThrow(
                () -> new IllegalArgumentException("bean expected lacks the property " + property))
            .getReadMethod()
            .invoke(expected);
  ...
  private GeoServerTileLayerInfo testMarshaling(GeoServerTileLayerInfo info) throws Exception {
    ...
    XStream xstream =
        XMLConfiguration.getConfiguredXStream(new SecureXStream(), (WebApplicationContext) null);
