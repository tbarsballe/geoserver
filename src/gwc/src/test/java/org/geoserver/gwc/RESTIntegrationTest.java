...
public class RESTIntegrationTest extends GeoServerSystemTestSupport {
  ...
  /**
   * PUT creates a new layer, shall fail if the layer id is not provided, the layer name is, but no
   * such layer is found in the {@link Catalog}
   */
  @Test
  ...
  public void testPutBadLayerEndpoint() throws Exception {
    ...
    assertEquals(
        expected,
        response.getContentAsString().substring(response.getContentAsString().indexOf(":") + 2));
  }
              ...
              allOf(
                  ...
                  isA(
                      (Class<ParameterFilter>)
                          StyleParameterFilter.class.asSubclass(ParameterFilter.class)))));
    }
    {
      final String xml =
          "<GeoServerLayer>" //
