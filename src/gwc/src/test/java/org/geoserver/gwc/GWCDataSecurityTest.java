...
public class GWCDataSecurityTest extends WMSTestSupport {
  ...
  protected void onSetUp(SystemTestData testData) throws Exception {
    ...
    tam.putLimits(
        "cite_mosaic2", coverage2, new DataAccessLimits(CatalogMode.CHALLENGE, Filter.INCLUDE));
    ...
    MultiPolygon cropper =
        (MultiPolygon) wkt.read("MULTIPOLYGON(((140 -50, 150 -50, 150 -30, 140 -30, 140 -50)))");
    tam.putLimits(
        "cite_cropmosaic",
        coverage,
  ...
  protected void doPermissionCropTileTest(
    ...
    assertThat(
        ...
        addBodyOnFail(
            hasProperty(
                "contentType",
                equalTo(tileFormat)))); // If this was unsuccessful the test is invalid
  ...
  protected void doPermissionMosaicTileTest(
    ...
    // mode hide
    assertThat(
        string(getBinaryInputStream(response)), containsString("Could not find layer sf:mosaic"));
  }
  ...
  public void testPermissionMosaicTileGmaps() throws Exception {
    doPermissionMosaicTileTest(
        (layer) ->
            String.format("gwc/service/gmaps?LAYERS=%s&FORMAT=image/png&ZOOM=0&X=0&Y=0", layer),
        SECURITY_ERROR_TYPE,
        NOT_FOUND_ERROR_TYPE);
  }
  ...
  public void testPermissionMosaicTileMGmaps() throws Exception {
    doPermissionMosaicTileTest(
        (layer) ->
            String.format("gwc/service/mgmaps?LAYERS=%s&FORMAT=image/png&ZOOM=17&X=0&Y=0", layer),
        SECURITY_ERROR_TYPE,
        NOT_FOUND_ERROR_TYPE);
  }
    ...
    System.out.println(
        Arrays.toString(
            GWC.get()
                ...
                .getCoverage(13)));
    System.out.println(
        GWC.get().getTileLayerByName("sf:mosaic").getGridSubset("EPSG:900913").getOriginalExtent());
    System.out.println(
        ((LayerInfo)
    ...
    doPermissionCropTileTest(
        (layer, index) -> {
          ...
          assertThat(
              ...
              equalTo(
                  index)); // Check that the test key is correct. Failure means the test is broken
  ...
  public void testLayerGroup() throws Exception {
    ...
    String str = string(getBinaryInputStream(response));
    assertTrue(
        str.contains("org.geotools.ows.ServiceException: Could not find layer " + NATURE_GROUP));
