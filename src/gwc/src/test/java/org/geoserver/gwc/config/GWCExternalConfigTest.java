...
public final class GWCExternalConfigTest extends GeoServerSystemTestSupport {
  ...
  /**
   * Helper method that setup the correct configuration variables, force Spring beans to be reloaded
   * and checks GWC configuration beans.
   */
        ...
        .forEach(
            bean -> {
              try {
                ...
                assertThat(
                    bean.getConfigDirectory().dir().getCanonicalPath(), is(expectedConfigFirPath));
                // rely on canonical path for comparisons
                assertThat(
                    new File(bean.getLocation()).getCanonicalPath(),
                    is(
                        new File(expectedConfigFirPath, bean.getConfigFileName())
                            .getCanonicalPath()));
              } catch (Exception exception) {
