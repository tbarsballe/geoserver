...
public class DefaultWebCoverageService111 implements WebCoverageService111 {
    ...
    try {
      ...
      if (gridCRS != null) {
        ...
        if (gridCRS.getGridType().equals(GridType.GT2dSimpleGrid.getXmlConstant())) {
          ...
          tx =
              new AffineTransform(
                  offsets[0], offsets[1], offsets[2], offsets[3], origin[0], origin[1]);
        } else {
          tx =
              new AffineTransform(
                  offsets[0], offsets[4], offsets[1], offsets[3], origin[0], origin[1]);
              ...
              (int)
                  Math.round(
                      requestedEnvelope.getUpperCorner().getOrdinate(2)
                          - requestedEnvelope.getLowerCorner().getOrdinate(2));
          ...
          if (elevationLevels > 0) {
            ...
            elevations[0] =
                requestedEnvelope.getLowerCorner().getOrdinate(2); // TODO put the extrema
        ...
        if (offsets.length == 2) {
          ...
          AffineTransform tx =
              new AffineTransform(offsets[0], offsets[1], offsets[3], offsets[4], 0, 0);
  ...
  private Double[] estimateOffsets(
    ...
    if (!reprojectionNeeded) {
      if (gridCRS != null) {
        ...
        offsets =
            new Double[] {at.getScaleX(), at.getShearX(), 0d, at.getShearY(), at.getScaleY(), 0d};
      }
    } else {
  ...
  private void checkOutput(CoverageInfo meta, OutputType output) {
    ...
    if (gridCRS != null) {
      ...
      if (gridTypeValue != null) {
        ...
        if (type == null)
          throw new WcsException(
              "Unknown grid type " + gridTypeValue, InvalidParameterValue, "GridType");
        else if (type == GridType.GT2dGridIn3dCrs)
          throw new WcsException(
              "Unsupported grid type " + gridTypeValue, InvalidParameterValue, "GridType");
      }
  ...
  /**
   * Checks that the elements of the RangeSubset part of the request do make sense by comparing them
   * to the coverage metadata
   *
   * @param info
   * @param rangeSubset
   */
  private void checkRangeSubset(CoverageInfo info, RangeSubsetType rangeSubset) {
    ...
    if (rangeSubset.getFieldSubset().size() > 1) {
      throw new WcsException(
          "Multi field coverages are not supported yet", InvalidParameterValue, "RangeSubset");
    }
    ...
    if (field.getAxisSubset().size() > 1) {
      throw new WcsException(
          "Multi axis coverages are not supported yet", InvalidParameterValue, "RangeSubset");
