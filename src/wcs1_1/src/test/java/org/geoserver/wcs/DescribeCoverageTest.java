...
public class DescribeCoverageTest extends WCSTestSupport {
  ...
  public void testDescribeByIdentifiers() throws Exception {
    ...
    for (int i = 0; i < nodes.getLength(); i++) {
      ...
      dom =
          getAsDOM(
              "wcs?request=DescribeCoverage&service=WCS&version=1.1.1&identifiers=" + identifier);
  ...
  public void testDescribeUnknownCoverageKvp() throws Exception {
    Document dom =
        getAsDOM(BASEPATH + "?request=DescribeCoverage&service=WCS&version=1.1.1&identifiers=plop");
    // print(dom);
