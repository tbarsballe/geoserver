...
public class AuthenticationFilterTest extends AbstractAuthenticationProviderTest {
  ...
  public void testBasicAuth() throws Exception {
    ...
    request.addHeader(
        "Authorization",
        "Basic " + new String(Base64.encodeBytes((testUserName + ":wrongpass").getBytes())));
    getProxy().doFilter(request, response, chain);
    ...
    request.addHeader(
        ...
        "Basic "
            + new String(
                Base64.encodeBytes((GeoServerUser.ROOT_USERNAME + ":geoserver1").getBytes())));
    getProxy().doFilter(request, response, chain);
    tmp = response.getHeader("WWW-Authenticate");
    assertNotNull(tmp);
    assert (tmp.indexOf(GeoServerSecurityManager.REALM) != -1);
  ...
  public void testCredentialsFromHeader() throws Exception {
    ...
    request.addHeader(
        "X-Credentials", "private-user=" + GeoServerUser.ROOT_USERNAME + "&private-pw=geoserver1");
    getProxy().doFilter(request, response, chain);
  ...
  public void testDigestAuth() throws Exception {
    ...
    headerValue =
        clientDigestString(tmp, GeoServerUser.ROOT_USERNAME, "geoserver1", request.getMethod());
    request.addHeader("Authorization", headerValue);
    getProxy().doFilter(request, response, chain);
    tmp = response.getHeader("WWW-Authenticate");
    assertNotNull(tmp);
  ...
  public void testBasicAuthWithRememberMe() throws Exception {
    ...
    request.addHeader(
        "Authorization", "Basic " + new String(Base64.encodeBytes(("abc@xyz.com:abc").getBytes())));
    getProxy().doFilter(request, response, chain);
    assertEquals(HttpServletResponse.SC_OK, response.getStatus());
    SecurityContext ctx =
        (SecurityContext)
