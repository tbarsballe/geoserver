...
public class SecureCatalogImplFilterTest {
  ...
  static Matcher<FeatureTypeInfo> matchFT(String name, WorkspaceInfo ws) {
    return allOf(
        hasProperty("name", is(name)), hasProperty("store", hasProperty("workspace", is(ws))));
  }
  ...
  public void testFeatureTypeList() throws Exception {
    ...
    mockFTInfo =
        createMockFeatureType("bar", mockWSInfo, CatalogMode.HIDE, mockFilter, false, false);
    source.add(mockFTInfo);
    replay(mockFTInfo);

    mockFTInfo =
        createMockFeatureType("baz", mockWSInfo, CatalogMode.CHALLENGE, mockFilter, false, false);
    source.add(mockFTInfo);
    replay(mockFTInfo);
