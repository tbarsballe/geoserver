...
public abstract class AbstractAuthorizationTest extends SecureObjectsTest {
  ...
  public void setUp() throws Exception {
    rwUser =
        new TestingAuthenticationToken(
            ...
            "supersecret",
            Arrays.asList(
                new GrantedAuthority[] {new GeoServerRole("READER"), new GeoServerRole("WRITER")}));
    roUser =
        new TestingAuthenticationToken(
            "ro",
            "supersecret",
    ...
    root =
        new TestingAuthenticationToken(
            "admin",
            "geoserver",
            Arrays.asList(new GrantedAuthority[] {new GeoServerRole(SecureTreeNode.ROOT_ROLE)}));
    ...
    layerGroupWithSomeLockedLayer =
        buildLayerGroup(
            "layerGroupWithSomeLockedLayer", lineStyle, toppWs, statesLayer, roadsLayer);

    // container groups for testing group security
    namedTreeA =
        buildLayerGroup("namedTreeA", Mode.NAMED, null, statesLayer, roadsLayer, citiesLayer);
    nestedContainerE = buildLayerGroup("nestedContainerE", Mode.CONTAINER, null, forestsLayer);
    containerTreeB =
        buildLayerGroup(
            "containerTreeB", Mode.CONTAINER, null, roadsLayer, landmarksLayer, nestedContainerE);
  ...
  protected LayerInfo buildLayer(
      String name, WorkspaceInfo ws, Class<? extends ResourceInfo> resourceClass) throws Exception {
    ...
    if (resource instanceof FeatureTypeInfo) {
      expect(
              ((FeatureTypeInfo) resource)
                  .getFeatureSource((ProgressListener) anyObject(), (Hints) anyObject()))
          .andReturn(fs)
          .anyTimes();
    }
  ...
  protected void populateCatalog() {
    ...
    expect(catalog.getLayerGroupsByWorkspace("topp"))
        .andReturn(
            Arrays.asList(new LayerGroupInfo[] {layerGroupTopp, layerGroupWithSomeLockedLayer}))
        .anyTimes();
    ...
    expect(catalog.list(eq(LayerGroupInfo.class), anyObject(Filter.class)))
        .andAnswer(
            () -> {
              ...
              Iterator<LayerGroupInfo> it = groups.stream().filter(lg -> f.evaluate(lg)).iterator();
