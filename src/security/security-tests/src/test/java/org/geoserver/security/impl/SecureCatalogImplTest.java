...
public class SecureCatalogImplTest extends AbstractAuthorizationTest {
  ...
  public void testWideOpen() throws Exception {
    ...
    assertThatBoth(
        sc.getCoverages(), sc.list(CoverageInfo.class, Predicates.acceptAll()), equalTo(coverages));
    assertThatBoth(
        sc.getWorkspaces(),
  ...
  public void testLockedDown() throws Exception {
    ...
    assertThatBoth(
        sc.getFeatureTypes(), sc.list(FeatureTypeInfo.class, Predicates.acceptAll()), empty());
  ...
  public void testEoLayerGroupMustBeHiddenIfItsRootLayerIsHidden() throws Exception {
    ...
    expect(eoCatalog.list(eq(LayerGroupInfo.class), anyObject(Filter.class)))
        .andReturn(new CloseableIteratorAdapter<LayerGroupInfo>(Collections.emptyIterator()))
        .anyTimes();
    replay(eoCatalog);
  ...
  public void testSecurityFilterComplex() throws Exception {
    ...
    while (it1.hasNext()) {
      ...
      assertNotSame(
          "Unexpectedly found states with security filter " + security, next, statesLayer);
