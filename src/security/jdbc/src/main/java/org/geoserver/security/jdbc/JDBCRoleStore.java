...
public class JDBCRoleStore extends JDBCRoleService implements GeoServerRoleStore {
  ...
  /**
   * The identical connection is used until {@link #store()} or {@link #load()} is called. Within a
   * transaction it is not possible to use different connections.
   *
  ...
  public void setParentRole(GeoServerRole role, GeoServerRole parentRole) throws IOException {
    ...
    if (helper.isValidParent(
      ...
      throw new IOException(
          parentRole.getAuthority() + " is not a valid parent for " + role.getAuthority());
