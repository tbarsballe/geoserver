...
public class StyleController extends AbstractCatalogController {
   ...
   * requested format does not support encoding from a StyledLayerDescriptor, instead writes the raw
   * style to the style resource.
   *
  ...
  /**
   * Extracts an input stream representing a zipped directory containing an sld file and any number
   * of image files to a temporary location on the filesystem.
   *
  ...
  private void saveImageResources(File directory, String workspaceName) throws IOException {
    ...
    for (File imageFile : imageFiles) {
      IOUtils.copyStream(
          new FileInputStream(imageFile), stylesDir.get(imageFile.getName()).out(), true, true);
    }
  }
  ...
  /**
   * Verifies the workspace name (if not null) for a REST request, throwing an appropriate exception
   * if invalid
   *
   * @param workspaceName The workspace name. Ignored if null.
   * @throws RestException if the workspace name is not null and the workspace doesn't exist in the
   *     catalog
   */
  ...
  private void checkStyleResourceNotExists(StyleInfo info) throws RestException {
    // ensure that a existing resource does not already exist, because we may not want to overwrite
    // it
    ...
    if (dataDir.style(info).getType() != Resource.Type.UNDEFINED) {
      throw new RestException(
          "Style resource " + info.getFilename() + " already exists.", HttpStatus.FORBIDDEN);
    }
  }
}
