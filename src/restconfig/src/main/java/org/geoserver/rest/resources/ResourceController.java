...
public class ResourceController extends RestBaseController {
  ...
  static class ResourceControllerConfiguration {
    ...
    ContentNegotiationStrategy resourceContentNegotiationStrategy() {
      return webRequest -> {
        ...
        if (new PatternsRequestCondition("/resource", "/resource/**").getMatchingCondition(request)
            != null) {
   ...
   *
   * <p>All the inner Resource classes are data transfer object for representing resource metadata,
   * this method also can return direct access to resource contents.
   *
   * <p>Headers:
   *
   * <ul>
    ...
    if (operation == Operation.METADATA) {
      ...
      if (resource.getType() == Resource.Type.UNDEFINED) {
        ...
        if (request.getMethod().equals("HEAD")) {
          ...
          result =
              wrapObject(new ResourceDirectoryInfo(resource, request), ResourceDirectoryInfo.class);
        } else {
  ...
  public void resourceDelete(HttpServletRequest request) {
    ...
    if (!removed) {
      throw new RestException(
          "Resource '" + resource.path() + "' not removed", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
      ...
      if (!resource.path().isEmpty()) {
        parent =
            new ResourceParentInfo(
                ...
                new AtomLink(
                    href(resource.parent().path()), "alternate", getFormat(request).toString()));
      }
      ...
      for (Resource child : resource.list()) {
        children.add(
            new ResourceChildInfo(
                child.name(),
                new AtomLink(
                    href(child.path()), "alternate", getMediaType(child, request).toString())));
      }
    }
