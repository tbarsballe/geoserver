...
public class StyleControllerTest extends CatalogRESTTestSupport {
  ...
  public void testGetAllASJSON() throws Exception {
    ...
    List<StyleInfo> styles = catalog.getStyles();
    assertEquals(
        styles.size(), ((JSONObject) json).getJSONObject("styles").getJSONArray("style").size());
  }
  ...
  public void testPostAsSLDWithName() throws Exception {
    ...
    MockHttpServletResponse response =
        postAsServletResponse(
            RestBaseController.ROOT_PATH + "/styles?name=bar", xml, SLDHandler.MIMETYPE_10);
    assertEquals(201, response.getStatus());
    assertNotNull(response.getHeader("Location"));
  ...
  public void testPutAsSLD() throws Exception {
    ...
    MockHttpServletResponse response =
        putAsServletResponse(
            RestBaseController.ROOT_PATH + "/styles/Ponds", xml, SLDHandler.MIMETYPE_10);
    assertEquals(200, response.getStatus());
  ...
  public void testPutAsSLDWithExtension() throws Exception {
    ...
    MockHttpServletResponse response =
        putAsServletResponse(
            RestBaseController.ROOT_PATH + "/styles/Ponds.sld", xml, SLDHandler.MIMETYPE_10);
    assertEquals(200, response.getStatus());
  ...
  public void testPostByLayerWithDefault() throws Exception {
    ...
    MockHttpServletResponse response =
        postAsServletResponse(
            RestBaseController.ROOT_PATH + "/layers/cite:BasicPolygons/styles?default=true",
            xml,
            "text/xml");
    assertEquals(201, response.getStatus());
  ...
  public void testPostAsSE() throws Exception {
    ...
    MockHttpServletResponse response =
        postAsServletResponse(
            RestBaseController.ROOT_PATH + "/styles?name=foo", xml, SLDHandler.MIMETYPE_11);
    assertEquals(201, response.getStatus());
    assertNotNull(response.getHeader("Location"));
    assertTrue(response.getHeader("Location").endsWith("/styles/foo"));
   ...
   * Accept header is correct. - Maybe more importantly it could potentially break other end points
   * that depend on preferring the path extension.
   *
