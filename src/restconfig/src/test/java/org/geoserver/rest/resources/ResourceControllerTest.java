...
public class ResourceControllerTest extends GeoServerSystemTestSupport {
  ...
  public void testResourceMetadataXML() throws Exception {
    ...
    Document doc =
        getAsDOM(
            RestBaseController.ROOT_PATH + "/resource/mydir/myres?operation=mEtAdATa&format=xml");
    // print(doc);
  ...
  public void testResourceMetadataJSON() throws Exception {
    JSON json =
        getAsJSON(
            RestBaseController.ROOT_PATH + "/resource/mydir/myres?operation=metadata&format=json");
    // print(json);
    String expected =
        "{\"ResourceMetadata\": {"
            + "  \"name\": \"myres\","
  ...
  public void testResourceMetadataHTML() throws Exception {
    ...
    Document doc =
        getAsDOM(
            RestBaseController.ROOT_PATH + "/resource/mydir/myres?operation=metadata&format=html");
    // print(doc);
  ...
  public void testDirectoryHeaders() throws Exception {
    ...
    Assert.assertEquals(
        FORMAT_HEADER.format(myRes.parent().lastmodified()), response.getHeader("Last-Modified"));
    Assert.assertEquals(
        "http://localhost:8080/geoserver" + RestBaseController.ROOT_PATH + "/resource/",
        response.getHeader("Resource-Parent"));
