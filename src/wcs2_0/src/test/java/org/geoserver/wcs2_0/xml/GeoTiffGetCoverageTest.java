...
public class GeoTiffGetCoverageTest extends WCSTestSupport {
  ...
  public void testGeotiffExtensionCompressionJPEGWrongQuality1() throws Exception {
    final File xml =
        new File("./src/test/resources/geotiff/geotiffExtensionCompressionJPEGWrongQuality1.xml");
  ...
  public void testGeotiffExtensionCompressionJPEGWrongQuality2() throws Exception {
    final File xml =
        new File("./src/test/resources/geotiff/geotiffExtensionCompressionJPEGWrongQuality2.xml");
    final String request = FileUtils.readFileToString(xml);
  ...
  public void testGeotiffExtensionCompressionLZW() throws Exception {
    ...
    IIOMetadataNode root =
        (IIOMetadataNode)
            reader.getImageMetadata(0).getAsTree(TIFFImageMetadata.nativeMetadataFormatName);
    IIOMetadataNode field = getTiffField(root, BaselineTIFFTagSet.TAG_COMPRESSION);
    assertNotNull(field);
    assertEquals(
        "LZW", field.getFirstChild().getFirstChild().getAttributes().item(1).getNodeValue());
  ...
  public void testGeotiffExtensionCompressionDeflate() throws Exception {
    ...
    assertNotNull(field);
    assertEquals(
        "Deflate", field.getFirstChild().getFirstChild().getAttributes().item(1).getNodeValue());
    assertEquals(
        "32946", field.getFirstChild().getFirstChild().getAttributes().item(0).getNodeValue());
  ...
  public void testGeotiffExtensionCompressionPackBits() throws Exception {
    ...
    assertNotNull(field);
    assertEquals(
        "PackBits", field.getFirstChild().getFirstChild().getAttributes().item(1).getNodeValue());
    assertEquals(
        "32773", field.getFirstChild().getFirstChild().getAttributes().item(0).getNodeValue());
  ...
  public void testGeotiffExtensionCompressionWrongCompression() throws Exception {
    final File xml =
        new File("./src/test/resources/geotiff/geotiffExtensionCompressionWrongCompression.xml");
    final String request = FileUtils.readFileToString(xml);
  ...
  public void testGeotiffExtensionCompressionJPEG() throws Exception {
    ...
    assertNotNull(field);
    assertEquals(
        "JPEG", field.getFirstChild().getFirstChild().getAttributes().item(1).getNodeValue());
