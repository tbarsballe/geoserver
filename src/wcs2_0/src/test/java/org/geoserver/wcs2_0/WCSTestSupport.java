...
public abstract class WCSTestSupport extends GeoServerSystemTestSupport {
  ...
  static {
    ...
    try {
          ...
          new LSResourceResolver() {
...

            {
              try {
                ...
                dom =
                    (DOMImplementationLS)
                        builder.parse(new ByteArrayInputStream(xml.getBytes())).getImplementation();
              } catch (Exception e) {
      ...
      WCS20_SCHEMA =
          factory.newSchema(WCSTestSupport.class.getResource("/schemas/wcs/2.0/wcsAll.xsd"));
    } catch (Exception e) {
  ...
  protected static void assertEnvelopeEquals(
      GeneralEnvelope expected, double scaleExpected, GeneralEnvelope actual, double scaleActual) {
