...
public interface WCS20CoverageMetadataProvider {
   ...
   *
   * <p>The returned String array must consist of namespace,location pairs in which the namespace is
   * the full namespace uri of the schema, and location is the url to where the schema definition is
   * located.
   *
   * <p>The location may be specified as a canonical external url. For example
   * <tt>http://schemas.opengis.net/foo/foo.xsd</tt>. Or if the schema is bundled within the server
   * the location can be a relative path such as <tt>foo/foo.xsd</tt>. In the latter case the path
   * will be appended to the base url from which the capabilities document is being requested from.
   *
   * @param schemaBaseURL
   */
