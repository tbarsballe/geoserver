...
public class WCSDefaultValuesHelper {
   ...
   */
  private void sortBy(
      Query query, DimensionDescriptor timeDimension, DimensionDescriptor elevationDimension) {
  ...
  private Filter setFilters(
    ...
    Filter timeFilter =
        temporalSubset == null && timeDimension == null
            ? null
            : setTimeFilter(
                temporalSubset, timeDimension.getStartAttribute(), timeDimension.getEndAttribute());
   ...
   * @throws IOException
   */
  private Filter setEnevelopeFilter(Envelope envelopeSubset, StructuredGridCoverage2DReader reader)
      throws IOException {
  ...
  private Filter setAdditionalDimensionsFilter(
    ...
    // Check whether the number of specified additional dimensions values doesn't match the number
    // of available additional dimensions
    if (additionalDimensions != null
      ...
      for (String dimension : dimensionKeys) {
        ...
        Filter dimensionFilter =
            createCustomDimensionFilter(dimension, dimensionSubset, additionalDimensions);
  ...
  private GridCoverageRequest setStandardReaderDefaults(GridCoverageRequest subsettingRequest)
    ...
    // Reader is not a StructuredGridCoverage2DReader instance. Set default ones with policy "time =
    // max, elevation = min".
  ...
  private void setDefaultCustomDimensions(
      List<String> customDomains, Map<String, List<Object>> dimensionSubset) throws IOException {
