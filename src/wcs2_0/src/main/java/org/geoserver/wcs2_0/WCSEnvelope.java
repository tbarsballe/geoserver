...
public class WCSEnvelope extends AbstractEnvelope {
  ...
  /**
   * Sets the range for the given dimension. If the dimension is the longitude, it is allowed to set
   * a minimum greater than the maximum, this envelope will be assumed to span the dateline
   *
   * @param dimension
   * @param minimum
   * @param maximum
  ...
  /**
   * Returns true if the envelope has a empty span on at least one dimension. A span is empty if its
   * zero or negative, but in case a dimension is the longitude, a negative span will be treated as
   * a dateline crossing, and thus treated as non empty
   */
  ...
  /**
   * Returns a list of envelopes that avoid the dateline crossing "odd" representation, that is, in
   * that case two envelopes will be returned covering the portion before and after the dateline
   */
