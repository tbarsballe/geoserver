...
enum ScalingPolicy {
  ...
  ScaleToSize {
    ...
    /**
     * In this case we must retain the lower bounds by scale the size, hence {@link ScaleDescriptor}
     * JAI operation cannot be used. Same goes for {@link AffineDescriptor}, the only real option is
     * {@link WarpDescriptor}.
     *
     * @param wcsinfo
     */
  ...
  /**
   * Scale the provided {@link GridCoverage2D} according to the provided {@link ScalingType} and the
   * provided {@link Interpolation} and {@link Hints}.
   *
   ...
   * @param interpolation the {@link Interpolation} to use. In case it is <code>null</code> we will
   *     use the {@link InterpolationPolicy} default value.
  ...
  /**
   * Extract the requested targetSize from this scaling extension in case the provided scaling is a
   * ScaleToSizeType type.
   *
  ...
  /**
   * Extract the requested scaleFactors from this scaling extension in case the provided scaling is
   * a ScaleXXXFactor type.
   *
   * <p>Throw an {@link IllegalArgumentException} in case the scaling type is not a supported one.
   *
   * @param scaling
   ...
   *
   * <p>This is usually required when using overviews. Suppose you want to get a target scaleFactor
   * of 0.00001 and the worst overview provide you a scale factor of 0.0001, then the current
   * scaleFactor need to be adjusted by a remaining 0.1 factor.
   *
   * @param hints
   * @param scaleFactors
