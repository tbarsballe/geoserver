...
class GMLTransformer extends TransformerBase {
  ...
  class GMLTranslator extends TranslatorSupport {
     ...
     * easting,northing, hence in case of a northing,easting crs we use a reversed order to indicate
     * that we always walk on the raster columns first.
     *
    ...
    public void handleCoverageFunction(GridEnvelope2D gridRange, boolean axisSwap) {
      ...
      element(
          "gml:startPoint",
          gridRange.x + " " + gridRange.y); // we start at minx, miny (this is optional though)
    ...
    public void handleSampleDimensionNilValues(GridCoverage2D gc2d, double[] nodataValues) {
      ...
      if (nodataValues != null && nodataValues.length > 0) {
        ...
        if (noDataProperty != null) {
          String nodata =
              Double.valueOf(noDataProperty.getAsSingleValue()).toString(); // TODO test me
          final AttributesImpl nodataAttr = new AttributesImpl();
          nodataAttr.addAttribute(
              "", "reason", "reason", "", "http://www.opengis.net/def/nil/OGC/0/unknown");
