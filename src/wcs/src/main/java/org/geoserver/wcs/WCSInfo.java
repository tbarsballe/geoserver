...
public interface WCSInfo extends ServiceInfo {
  ...
  /**
   * Returns the maximum input size, in kilobytes. The input size is computed as the amount of bytes
   * needed to fully store in memory the input data, {code}width x heigth x pixelsize{code} (whether
   * that memory will actually be fully used or not depends on the data source) A negative or null
   * value implies there is no input limit.
   */
