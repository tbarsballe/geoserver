...
public class WCSUtils {
        ...
        if (param.equalsIgnoreCase("BAND")) {
          try {
            ...
            throw new WcsException(
                "Band parameters incorrectly specified: " + e.getLocalizedMessage());
          }
        }
      }
    }
  ...
  public static Coverage bandSelect(final GridCoverage coverage, final int[] bands) {
    ...
    if ((bands != null) && (bands.length > 0)) {
      ...
      final ParameterValueGroup param =
          (ParameterValueGroup) PROCESSOR.getOperation("SelectSampleDimension").getParameters();
      param.parameter("Source").setValue(coverage);
   ...
   * Mind, this method might cause the coverage to be fully read in memory (if that is the case, the
   * actual WCS processing chain would result in the same behavior so this is not causing any extra
   * memory usage, just makes it happen sooner)
   *
   * @param coverage
   */
   ...
   * limits. This method has to jump through a few hoops to estimate the size of the data to be read
   * without having to actually read the coverage (which might trigger the loading of the full
   * coverage in memory)
   *
   * @param meta
   * @param reader
  ...
  /**
   * Returns an eventual filter included among the parsed kvp map of the current request. Will work
   * for CQL_FILTER, FILTER and FEATURE_ID
   */
