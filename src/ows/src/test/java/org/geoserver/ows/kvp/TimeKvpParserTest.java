...
public class TimeKvpParserTest extends TestCase {
  ...
  public void testReducedAccuracyHour() throws Exception {
    ...
    hour =
        (DateRange)
            TimeParser.getFuzzyDate("2005-12-31T23Z"); // selected due to leapsecond at 23:59:60 UTC
    c.clear();
    ...
    instant =
        (Date)
            TimeParser.getFuzzyDate(
                "2005-12-31T23:59:60.000Z"); // selected due to leapsecond at 23:59:60 UTC
    c.clear();
    c.set(Calendar.YEAR, 2005);
    c.set(Calendar.MONTH, 11);
    c.set(Calendar.DAY_OF_MONTH, 31);
  ...
  public void testMixedValues() throws ParseException {
    ...
    assertRange(
        (DateRange) l.get(1), format.parse("2007-02-01T12Z"), format.parse("2007-02-01T13Z"));
  }
  ...
  public void testOrderedValues() throws Exception {
    ...
    assertTrue(l.size() == 5);
    assertRange(
        (DateRange) l.get(0), format.parse("2007-01-01T12Z"), format.parse("2007-01-01T13Z"));
    assertRange(
        (DateRange) l.get(1), format.parse("2007-01-05T12Z"), format.parse("2007-01-05T13Z"));
    assertRange(
        (DateRange) l.get(2), format.parse("2007-01-12T12Z"), format.parse("2007-01-12T13Z"));
    assertRange(
        (DateRange) l.get(3), format.parse("2007-01-17T12Z"), format.parse("2007-01-17T13Z"));
    assertRange(
        (DateRange) l.get(4), format.parse("2007-01-29T12Z"), format.parse("2007-01-29T13Z"));
  }
