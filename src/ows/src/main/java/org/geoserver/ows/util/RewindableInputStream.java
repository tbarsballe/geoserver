...
public class RewindableInputStream extends InputStream {
  ...
  /**
   * Internal buffer of bytes already read from source input stream. Allows to access the same byte
   * more than once.
   */
   ...
   * RewindableInputStream</code> instance stream's "end" could be reached more than once, so it is
   * a good thing to know where original stream ended to avoid <code>IOExceptions</code>.
   */
  protected int fEndOffset;

  /**
   * Offset of the next byte to be read from the stream relative to the beginning of the stream (and
   * <code>fData</code> array as well)
   */
  ...
  /**
   * Creates new <code>RewindableInputStream</code> object with internal buffer of default size and
   * default value of chunked reading flag (which is _currently_ <code>true</code>).
   *
   ...
   *     bytes can be read from the underlying stream in single reading operation or not. This value
   *     can be changed using <code>setChunkedMode</code> (or its aliases). For specific purpose of
   *     inferring encoding/charset of XML document typical usage policy is to disable chunked reads
   *     while obtaining XML declaration and then enable it to speed up reading the rest of
   *     document.
   ...
   * allowed chunked reading and then read some data from the stream, you better forget about <code>
   * reset</code>ting or <code>rewind</code>ing it after that.
   *
  ...
  /**
   * Reads next byte from this stream. This byte is either being read from underlying InputStream or
   * taken from the internal buffer in case it was already read at some point before.
   *
   ...
   * <code>fMayReadChunks == false</code>). After reaching the end of the buffer, each invocation of
   * this method will read exactly 1 byte then. In "chunked" mode this method <em>may</em> return
   * more than 1 byte, but it doesn't buffer the result.
   *
   * <p>From the other hand, for the task of reading xml declaration, such behavior may be
   * desirable, as we probably don't need reset/rewind functionality after we finished with charset
   * deduction. It is good idea to call <code>enableChunkedMode</code> after that, in order to
   * improve perfomance and lessen memoery consumption when reading the rest of the data.
   *
   ...
   * <code>0</code>. The actual number of bytes skipped is returned. If <code>n</code> is negative,
   ...
   * blocking by the next caller of a method for this input stream. For <code>RewindableInputStream
   * </code> this can be:
   *
   * <ul>
  ...
  /**
   * Returns stream pointer to the position previously remembered using <code>mark</code> method (or
   * to beginning of the stream, if there were no <code>mark</code> method calls).
   */
  public void reset() {
    fOffset = fMark;
  }
