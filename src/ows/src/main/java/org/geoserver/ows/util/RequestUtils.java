...
public class RequestUtils {
  ...
  /**
   * Given a list of provided versions, and a list of accepted versions, this method will return the
   * negotiated version to be used for response according to the pre OWS 1.1 specifications, that
   * is, WMS 1.1, WMS 1.3, WFS 1.0, WFS 1.1 and WCS 1.0
   *
  ...
  /**
   * Given a list of provided versions, and a list of accepted versions, this method will return the
   * negotiated version to be used for response according to the OWS 1.1 specification (at the time
   * of writing, only WCS 1.1.1 is using it)
   *
   * @param providedList a non null, non empty list of provided versions (in "x.y.z" format)
   * @param acceptedList a list of accepted versions, eventually null or empty (in "x.y.z" format)
   * @return the negotiated version to be used for response
  ...
  /**
   * Checks the validity of a version number (the specification version numbers, three dot separated
   * integers between 0 and 99). Throws a ServiceException if the version number is not valid.
   *
   ...
   * preparse some of the xml document, resetting it back to its original state for actual parsing.
   *
   * @param stream The original xml stream.
   * @param xmlLookahead The number of bytes to support for parse. If more than this number of bytes
   *     are preparsed the stream can not be properly reset.
