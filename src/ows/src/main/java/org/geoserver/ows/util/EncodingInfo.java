...
public class EncodingInfo {
   ...
   * </code> if order is big-endian, <code>false</code> for little-endian, and <code>null</code> if
   * byte order is not relevant for this encoding scheme. This is a "three-state" switch (third is
   * <code>null</code>), so it can't be just plain <code>boolean</code> type.
   */
  private Boolean fIsBigEndian = null;

  /**
   * This is technically not a part of encoding metadata, but more like characteristic of the input
   * XML document. Tells whether Byte Order Mark (BOM) was found while detecting encoding scheme.
   */
  private boolean fHasBOM;

  /**
   * Non-arg constructor to use in a few cases when you need a blank instance of <code>EncodingInfo
   * </code>. It cant' be used right after creation and should be populated first via either setters
   * or specific charset detection methods.
   */
  public EncodingInfo() {}

  /**
   * Constructor that takes name of the encoding scheme and endianness - results of autodetection in
   * <code>getEncodingName</code>. BOM is considered missing if object is constructed this way.
   *
  ...
  /**
   * Constructor that takes name of the encoding scheme and endianness - results of autodetection in
   * <code>getEncodingName()</code>. Also presence of Byte Order Mark should be specified
   * explicitly.
   *
   * @param encoding Name of the autodetected encoding scheme.
   * @param isBigEndian
   ...
   * method is currently used (at least it was :) in <code>XmlCharsetDetector.getCharsetAwareReader
   * </code> (other two ways simply don't work).
   *
