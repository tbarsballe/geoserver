  ...
  public Collection parse(String value) throws ParseException {
    ...
    for (String date : listDates) {
      ...
      if (date.indexOf("/") <= 0) {
        ...
        // May be one of the following possible ISO 8601 Time Interval formats with trailing period
        // for
          ...
          while ((time = j * millisIncrement + startTime) <= endTime) {
            ...
            if (j >= MAX_ELEMENTS_TIMES_KVP) {
              if (LOGGER.isLoggable(Level.INFO))
                LOGGER.info(
                    "Lmiting number of elements in this periodo to " + MAX_ELEMENTS_TIMES_KVP);
              break;
            }
          }
        }
        ...
        // May be one of the following possible ISO 8601 Time Interval formats, as in ECQL Time
        // Period:
        // TIME/DURATION
        // DURATION/TIME
        // TIME/TIME
