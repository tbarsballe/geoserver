...
public class ElevationParser {
   ...
   * containing only the parsed value. In the second case, this method returns a list including all
   * elevations from start value up to the end value with the interval specified in the {@code
   * value} string.
   *
          ...
          while ((step = j * increment + begin) <= end) {
            ...
            if (j >= MAX_ELEMENTS_ELEVATIONS_KVP) {
              if (LOGGER.isLoggable(Level.INFO))
                LOGGER.info(
                    "Lmiting number of elements in this periodo to " + MAX_ELEMENTS_ELEVATIONS_KVP);
              break;
            }
          }
        } else {
