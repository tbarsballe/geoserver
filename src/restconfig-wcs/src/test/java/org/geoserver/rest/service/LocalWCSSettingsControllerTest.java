...
public class LocalWCSSettingsControllerTest extends CatalogRESTTestSupport {
  ...
  public void testGetAsJSON() throws Exception {
    JSON json =
        getAsJSON(RestBaseController.ROOT_PATH + "/services/wcs/workspaces/sf/settings.json");
  ...
  public void testPutFullAsXML() throws Exception {
    String xml =
        IOUtils.toString(LocalWCSSettingsControllerTest.class.getResourceAsStream("wcs.xml"));
    MockHttpServletResponse response =
        putAsServletResponse(
            RestBaseController.ROOT_PATH + "/services/wcs/workspaces/sf/settings", xml, "text/xml");
    assertEquals(200, response.getStatus());
