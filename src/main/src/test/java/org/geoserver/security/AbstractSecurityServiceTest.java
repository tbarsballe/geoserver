...
public abstract class AbstractSecurityServiceTest extends GeoServerSystemTestSupport {
  ...
  protected void checkValuesInserted(GeoServerRoleService roleService) throws IOException {
    ...
    assertEquals(
        roleService.getParentRole(role_wfs).getProperties().getProperty("bbox"), "lookupAtRuntime");
    ...
    assertEquals(
        roleService.getParentMappings().get(role_wfs.getAuthority()), role_auth.getAuthority());
    assertEquals(
        roleService.getParentMappings().get(role_wms.getAuthority()), role_auth.getAuthority());
  }
  ...
  protected void checkValuesInserted(GeoServerUserGroupService userGroupService)
    ...
    assertEquals(
        userGroupService.getUserCount(), userGroupService.getUserCountNotHavingProperty("unknown"));
    ...
    assertEquals(
        userGroupService.getUserCount() - 1, userGroupService.getUserCountNotHavingProperty("tel"));
  ...
  /**
   * Indicates if the test is a JDBC test All test are based on the fact that locking appears at row
   * level. This is not true for JDBC databases - Locking may be based on blocks - Since only a few
   * records are in the table, the whole table my be locked. (lock escalation)
   *
