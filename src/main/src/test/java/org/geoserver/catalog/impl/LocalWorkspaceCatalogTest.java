...
public class LocalWorkspaceCatalogTest {
  ...
  public void setUp() throws Exception {
    ...
    layers2.add(null);
    expect(cat.list(LayerInfo.class, Filter.INCLUDE, (Integer) null, (Integer) null, (SortBy) null))
  ...
  /**
   * No geoserver instance has been set. This means there is no access to geoserver. this should not
   * happen but we want to protect against this consideration. In this case we have a local
  ...
  /**
   * No localworkspace so prefix should be included since the global capabilities is probably being
   * created.
   */
  @Test
  ...
  private void checkLayerNamePrefixInclusion(boolean includePrefix, Iterator<LayerInfo> layers) {
    while (layers.hasNext()) {
      ...
      if (includePrefix) {
        message =
            layerInfo.getName() + " should contain a : because the prefix should have been kept";
      } else {
        message =
            layerInfo.getName()
