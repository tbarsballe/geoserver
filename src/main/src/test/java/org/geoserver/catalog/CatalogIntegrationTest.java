...
public class CatalogIntegrationTest extends GeoServerSystemTestSupport {
  ...
  protected void onSetUp(SystemTestData testData) throws IOException {
    testData.addStyle(
        "singleStyleGroup", "singleStyleGroup.sld", CatalogIntegrationTest.class, getCatalog());
    testData.addStyle(
        "multiStyleGroup", "multiStyleGroup.sld", CatalogIntegrationTest.class, getCatalog());
    testData.addStyle(
        "recursiveStyleGroup",
  ...
  public void testSingleStyleGroup() throws Exception {
    ...
    assertEquals(
        catalog.getLayerByName((getLayerId(MockData.STREAMS))).getResource().getLatLonBoundingBox(),
        lg.getBounds());
  }
  ...
  public void testMultiStyleGroup() throws Exception {
    ...
    assertTrue(
        styles.get(0).getStyle().featureTypeStyles().get(0).rules().get(0).getSymbolizers()[0]
    ...
    assertTrue(
        styles.get(1).getStyle().featureTypeStyles().get(0).rules().get(0).getSymbolizers()[0]
            instanceof LineSymbolizer);
    ...
    assertTrue(
        styles.get(5).getStyle().featureTypeStyles().get(0).rules().get(0).getSymbolizers()[0]
            instanceof PointSymbolizer);

    // Test bounds calculation
    new LayerGroupHelper(lg).calculateBounds();
    assertEquals(
        new ReferencedEnvelope(-180, 180, -90, 90, DefaultGeographicCRS.WGS84), lg.getBounds());
  }
