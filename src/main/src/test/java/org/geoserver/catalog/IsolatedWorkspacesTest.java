...
public abstract class IsolatedWorkspacesTest extends GeoServerSystemTestSupport {
  ...
  /**
   * Helper functional interface to allow passing functions that don't receive anything as input and
   * don't provide anything as output.
   */
  @FunctionalInterface
   ...
   * happen. This method will check hat the obtained exception contains the expected message and is
   * an instance expected type.
   */
  ...
  /**
   * Helper method that creates a workspace and add it to the catalog. This method will first create
   * the namespace and then the workspace. The create workspaces prefixes are stored in {@link
   * #CREATED_WORKSPACES_PREFIXES}.
   */
