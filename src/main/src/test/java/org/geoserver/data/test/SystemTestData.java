...
public class SystemTestData extends CiteTestData {
  ...
  public void setUpSecurity() throws IOException {
    ...
    if (javaVendor.contains("IBM")) {
      ...
      IOUtils.copy(
          new File(secDir, "geoserver.jceks.default"), new File(secDir, "geoserver.jceks"));
    }
  }
  ...
  protected void createConfig() {
    ...
    geoServer.addListener(
        new GeoServerPersister(new GeoServerResourceLoader(data), createXStreamPersister()));
  ...
  /**
   * Adds a workspace to the test setup. If the workspace and namespace already exists they will be
   * updated, otherwise they will be created.
   *
   ...
   *
   * <p>To set up the style a file named <tt>filename</tt> is copied from the classpath relative to
   * the <tt>scope</tt> parameter. Example: "../temperature.sld" is copied to
   ...
   * Adds a vector layer to the catalog setup.
   *
   * <p>This method calls through to {@link #addVectorLayer(QName, Map, Class, Catalog)} passing in
   ...
   * qName.getLocalPart()</code>.properties". The <tt>scope</tt> parameter is used as the class from
   ...
   * This method assumes the scope of this class and calls through to {@link #addRasterLayer(QName,
   * String, String, Map, Class, Catalog)}
   */
    ...
    try {
      ...
      if (reader instanceof StructuredGridCoverage2DReader
        ...
        for (String coverageName : coverageNames) {
          addCoverage(
              store, builder, reader, catalog, format, coverageName, qName, props, coverageName);
        }
      } else {
  ...
  public void addSettings(String workspace, GeoServer geoServer) {
    ...
    settings
        .getContact()
        .setAddressDeliveryPoint("1600 Pennsylvania Ave NW, Washington DC 20500, United States");
        ...
        try {
          ...
          if (i >= MAX_ATTEMPTS) {
            throw new IOException(
                "Failed to clean up test data dir after " + MAX_ATTEMPTS + " attempts", e);
          }
          System.err.println(
