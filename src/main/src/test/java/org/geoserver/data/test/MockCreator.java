  ...
  public GeoServerSecurityManager createSecurityManager(MockTestData testData) throws Exception {
    ...
    expect(secMgr.listPasswordValidators())
        .andReturn(
            new TreeSet<String>(
                Arrays.asList(
                    PasswordValidator.DEFAULT_NAME, PasswordValidator.MASTERPASSWORD_NAME)))
        .anyTimes();
    ;
    ...
    expect(secMgr.listAuthenticationProviders())
        .andReturn(new TreeSet<String>(Arrays.asList(GeoServerAuthenticationProvider.DEFAULT_NAME)))
        .anyTimes();
