...
public class GeoServerDataDirectoryTest {
  ...
  public void testNullWorkspace() {
    ...
    assertEquals(
        dataDir.getStyles((WorkspaceInfo) null, "test").path(), dataDir.getStyles("test").path());
    assertEquals(
  ...
  public void testParsedStyleExternalWithParams() throws IOException {
    ...
    assertTrue(graphic instanceof ExternalGraphic);
    assertEquals(
        ((ExternalGraphic) graphic).getLocation().getPath(), iconFile.toURI().toURL().getPath());
  ...
  /**
   * Test loading a parsed style with an external graphic URL that contains both ?queryParams and a
   * URL #fragment, and assert that those URL components are preserved.
   *
   * @throws IOException
   */
  @Test
