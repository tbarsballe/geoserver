    ...
    public GridCoverageWrapper(
      ...
      wrappedPropertySource =
          new PropertySourceImpl(
              properties, coverage instanceof PropertySource ? (PropertySource) coverage : null);
    }
  ...
  static class WrappedSampleDimension extends GridSampleDimension implements SampleDimension {
    ...
    public static WrappedSampleDimension build(
      ...
      InternationalString configuredDescription =
          (sampleDimDescription == null || !sampleDimDescription.toString().equalsIgnoreCase(name))
        ...
        for (Category category : categories) {
          ...
          if (Category.NODATA.getName().equals(category.getName())) {
            if (category.isQuantitative()) {
              ...
              double minimum =
                  nodataConfigured ? configuredNoDataValues[0] : category.getRange().getMinimum();
              double maximum =
                  nodataConfigured ? configuredNoDataValues[0] : category.getRange().getMaximum();
      ...
      // Adding the full data range, it's mandatory to have it if we want the configured range to
      // survive a grid dimension copy
