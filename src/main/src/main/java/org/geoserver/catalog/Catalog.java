...
public interface Catalog extends CatalogInfo {
   ...
   *
   * <p>In the even the specified object does not exist in the catalog it itself should be returned,
   ...
   * </pre>
   *
   * @param name The name of the store. The name can be {@code null} or {@link #DEFAULT} to retrieve
   *     the default store in the default workspace.
   * @param clazz The class of the store to return.
   ...
   * @param name The name of the store. The name can be {@code null} or {@link #DEFAULT} to retrieve
   *     the default store in the specified workspace.
  ...
  /**
   * Returns a datastore matching a particular id, or <code>null</code> if no such data store could
   * be found.
   *
   * <p>This method is convenience for:
   *
  ...
  /**
   * Returns the coveragestore matching a particular name in the specified workspace, or <code>null
   * </code> if no such coveragestore could be found.
   *
   * <p>This method is convenience for:
   *
   * <pre>
   ...
   * @param workspaceName The name of the workspace containing the coveragestore, may be <code>null
   *     </code> to specify the default workspace.
   ...
   * returned.
   *
   * @param ns The namespace to which the resource belongs, may be <code>null</code> to specify the
   *     default namespace.
   * @param name The name of the resource.
   * @param clazz The class of the resource.
   * @return The resource matching the name, or <code>null</code> if no such resource exists.
   ...
   *       returned.
   *   <li>If a single resource among all non-default namespaces is found matching the the specified
   *       name, it is returned.
   * </ul>
   *
  ...
  /**
   * Returns the feature type matching a particular id, or <code>null</code> if no such feature type
   * could be found.
   *
   * <p>This method is convenience for:
   *
   ...
   * </pre>
   *
   * @param ns The namespace to which the feature type belongs, may be <code>null</code> to specify
   ...
   *       returned.
   *   <li>If a single coverage among all non-default namespaces is found matching the the specified
   *       name, it is returned.
   * </ul>
   *
   ...
   *
   * <p>If {@code prefixedName} contains a workspace name prefix (like in {@code topp:tasmania}, the
   ...
   * </code> if no such layer group could be found.
   *
   * @param workspace The workspace containing the layer group, {@code null} is allowed, meaning to
   ...
   *
   * @param workspaceName The name of the workspace containing the style, {@code null} stands for a
   *     global style.
   ...
   * methods {@link #add(NamespaceInfo)} and {@link #remove(NamespaceInfo)} should be used for that
   * purpose.
   */
   ...
   * methods {@link #add(WorkspaceInfo)} and {@link #remove(WorkspaceInfo)} should be used for that
   * purpose.
   */
   ...
   * catalog subsystem.
   */
  void fireModified(CatalogInfo object, List<String> propertyNames, List oldValues, List newValues);
  ...
  /**
   * Returns an {@link Iterator} over the catalog objects of the requested type that match the given
   * query predicate, positioned at the specified {@code offset} and limited to the number requested
   * number of elements.
   *
   * <p>The returned iterator <strong>shall</strong> be closed once it is no longer needed, to
   * account for streaming implementations of this interface to release any needed resource such as
   * database or remote service connections. Example usage:
   *
   * <pre>
   * <code>
   * Catalog catalog = ...
   ...
   *     otherwise an integer {@code >= 0} to return an iterator positioned at the specified offset.
   * @param count {@code null} to return a non limited in number of elements iterator, an integer
   *     {@code >= 0} otherwise to specify the maximum number of elements the iterator shall return.
  ...
  /**
   * Return the catalog capabilities supported by this catalog. Normally this will correspond to the
   * capabilities supported by the used catalog facade.
   *
