...
class ModificationProxyCloner {
  ...
  /**
   * Best effort object cloning utility, tries different lightweight strategies, then falls back on
   * copy by XStream serialization (we use that one as we have a number of hooks to avoid deep
  ...
  static <T> T clone(T source) {
    ...
    if (source instanceof CatalogInfo) {
      ...
      return (T)
          ModificationProxy.create(source, getDeepestCatalogInfoInterface((CatalogInfo) source));
    }
    ...
    if (copyConstructor != null) {
      try {
        ...
        LOGGER.log(
            Level.FINE, "Source has a copy constructor, but it failed, skipping to XStream", e);
      }
    }
   ...
   *
   * @param source
   * @param deepCopy If true, a deep copy will be done, otherwise the cloned collection will contain
   *     the exact same objects as the source
