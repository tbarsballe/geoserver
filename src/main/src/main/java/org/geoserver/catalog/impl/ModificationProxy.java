...
public class ModificationProxy implements WrappingProxy, Serializable {
  ...
  private Collection cloneCollection(Collection oldCollection) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Unexpected failure while cloning collection of class " + oldCollectionClass, e);
    }
  }
   ...
   *
   * <p>This method is safe in that if the object passed in is not a proxy it is simply returned. If
   * the proxy is not an instance of {@link ModificationProxy} it is also returned untouched.
   */
