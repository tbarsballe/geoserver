...
public abstract class GeoServerSLDVisitor extends AbstractStyleVisitor {
   ...
   * multiple layers, such as a remote OWS user layer exposing multiple layers, this will be called
   * for once of each of these layers, which will be passed in through the info argument.
   *
   ...
   *
   * <p>Note: currently, in the case of a named layer referencing a layer group, any styles defined
   * in the SLD will be ignored, and the layer groups defined styles will be used instead, being
   * handled as user layers.
   *
   ...
   * references multiple layers, such as a named layer referencing a layer group, or remote OWS user
   * layer exposing multiple layers, visit each style for each of these sublayers instead.
   *
   ...
   * @throws UncheckedIOException if there is an underlying {@link IOException} when reading {@link
   *     Style} objects from the catalog
   */
  @Override
  ...
  public void visit(UserLayer layer) {
    ...
    if (layer.getRemoteOWS() != null) {
      ...
      // TODO: By the SLD spec, we shouldn't be supporting UserLayers as NamedLayers, but we do
      // anyways.
      setLayerState(visitNamedLayerInternal(layer), layer);
    }
    super.visit(layer);
   ...
   * each feature matching one of the supplied {@link FeatureTypeConstraint}s in a {@link LayerInfo}
   *
   * @param ul
