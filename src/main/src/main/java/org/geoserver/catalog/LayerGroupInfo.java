...
public interface LayerGroupInfo extends PublishedInfo {
  ...
  public enum Mode {
    /**
     * The layer group is seen as a single exposed layer with a name, does not actually contain the
     * layers it's referencing
     */
    SINGLE {
    ...
    /**
     * The layer group is seen as a single exposed layer with a name, but contains the layers it's
     * referencing, thus hiding them from the caps document unless also shown in other tree mode
     * layers
     */
    OPAQUE_CONTAINER {
      public String getName() {
    ...
    /**
     * The layer group retains a Name in the layer tree, but also exposes its nested layers in the
    ...
    /**
     * The layer group is exposed in the tree, but does not have a Name element, showing structure
     * but making it impossible to get all the layers at once.
     */
    CONTAINER {
      public String getName() {
  ...
  /**
   * A way to compare two LayerGroupInfo instances that works around all the wrappers we have around
   * (secured, decorating ecc) all changing some aspects of the bean and breaking usage of "common"
   * equality). This method only uses getters to fetch the fields. Could have been build using
   * EqualsBuilder and reflection, but would have been very slow and we do lots of these calls on
   * large catalogs.
   *
   * @param lg
   * @param obj
  ...
  public static int hashCode(LayerGroupInfo lg) {
    ...
    result =
        prime * result + ((lg.getRootLayerStyle() == null) ? 0 : lg.getRootLayerStyle().hashCode());
    result =
        prime * result + ((lg.getAuthorityURLs() == null) ? 0 : lg.getAuthorityURLs().hashCode());
    ...
    result =
        prime * result + ((lg.getMetadataLinks() == null) ? 0 : lg.getMetadataLinks().hashCode());
