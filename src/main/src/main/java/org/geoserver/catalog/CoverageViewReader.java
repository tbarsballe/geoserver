...
public class CoverageViewReader implements GridCoverage2DReader {
  ...
  public GridCoverage2D read(GeneralParameterValue[] parameters)
    ...
    for (int bIdx : selectedBandIndices) {
      ...
      if (!inputCoverages.containsKey(coverageName)) {
        ...
        // bands selection parameter inside on final bands so they should not be propagated to the
        // delegate reader
    ...
    if (nonNullCoverages < inputCoverages.size()) {
      ...
      for (String name : inputCoverages.keySet()) {
        ...
        ConstantDescriptor.create(
            width, height, bandValues, new RenderingHints(JAI.KEY_IMAGE_LAYOUT, layout));
      }
    }
    ...
    while (idx < selectedBandIndices.size()) {
      ...
      String coverageName =
          bands.get(selectedBandIndices.get(idx)).getInputCoverageBands().get(0).getCoverageName();

      if (idx + 1 < selectedBandIndices.size()
          && bands
              .get(selectedBandIndices.get(idx + 1))
  ...
  /**
   * Checks if a reader added a alpha channel on the fly as a result of a read parameter. We want to
   * preserve this alpha channel because the user never got a chance to select its presence in the
   * output (e.g. footprint management in mosaic)
   *
   * @param coverage
   * @param reader
   * @return
