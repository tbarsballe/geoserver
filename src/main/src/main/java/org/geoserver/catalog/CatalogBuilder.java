...
public class CatalogBuilder {
   ...
   *
   * <p>The resulting object is not added to the catalog, it must be done by the calling code after
   * the fact.
   */
   ...
   * </ul>
   *
   * TODO: make this method smarter, and compare the native crs to figure out if prejection actually
   * needs to be done, and sync it up with setting proj policy on coverage layers.
   */
   ...
   *       which this case take minutes are not uncommon if the data set is made of million of
   *       features)
  ...
  public ReferencedEnvelope getLatLonBounds(
      ReferencedEnvelope nativeBounds, CoordinateReferenceSystem declaredCRS) throws IOException {
   ...
   * and the reprojection policy in act
   *
   * @param rinfo
   ...
   *
   * @param ftinfo
   * @param extensive if true an extenstive lookup will be performed (more accurate, but might take
   *     various seconds)
   * @throws IOException
   */
   ...
   * @param specifiedName the published name for the coverage. If null, the name will be determined
   *     from the coverage store.
   * @return coverage for the specified name
   * @throws Exception if the coverage store was not found or could not be read, or if the coverage
  ...
  public CoverageInfo buildCoverage(
      GridCoverage2DReader reader, String coverageName, Map customParameters) throws Exception {
  ...
  private CoverageInfo buildCoverageInternal(
    ...
    cinfo
        ...
        .put(
            "dirName",
            new StringBuilder(store.getName()).append("_").append(nativeCoverageName).toString());
  ...
  private GridSampleDimension[] getCoverageSampleDimensions(
    ...
    if (gc != null) {
      ...
      if (imageLayout == null) {
        throw new Exception(
            "Unable to acquire test coverage and image layout for format:" + format.getName());
      }
      ColorModel cm = imageLayout.getColorModel(null);
      if (cm == null) {
        throw new Exception(
            "Unable to acquire test coverage and color model for format:" + format.getName());
      }
      SampleModel sm = imageLayout.getSampleModel(null);
      if (cm == null) {
        throw new Exception(
            "Unable to acquire test coverage and sample model for format:" + format.getName());
      }
    ...
    if (version.compareTo(new Version("1.3.0")) < 0) {
      ...
      if (srsName.startsWith("EPSG:")) {
        try {
          ...
          LOGGER.log(
              Level.WARNING,
              "Failed to determine axis order for " + srsName + ", assuming east/north",
              e);
          return false;
        }
      } else {
   ...
   *
   * <p>The <tt>recursive</tt> flag controls whether objects linked to the workspace such as stores
