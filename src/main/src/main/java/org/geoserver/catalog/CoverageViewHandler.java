...
class CoverageViewHandler {
  ...
  static class CoveragesConsistencyChecker {
    ...
    public CoveragesConsistencyChecker(
        GridCoverage2DReader reader, boolean canSupportHeterogeneousCoverages) throws IOException {
    ...
    public boolean checkConsistency(GridCoverage2DReader reader) throws IOException {
      ...
      final Rectangle thatRectangle =
          new Rectangle(
              gridRange.getLow(0), gridRange.getLow(1), gridRange.getSpan(0), gridRange.getSpan(1));
  ...
  /**
   * The coverageName to be used as reference. It is used for homogeneous coverages case, to extract
   * shared properties. It is used for heterogeneous coverages case, to access a specific coverage
   * in order to get the resolutions
   */
  ...
  public GridEnvelope getOriginalGridRange() {
    ...
    try {
      ...
      return new GridEnvelope2D(
          new Rectangle(
              (int) (envelope.getSpan(0) / res[0]), (int) (envelope.getSpan(1) / res[1])));
    } catch (IOException e) {
