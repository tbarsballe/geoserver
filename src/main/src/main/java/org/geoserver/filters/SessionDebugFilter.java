...
public class SessionDebugFilter implements Filter {
  ...
  class SessionDebugWrapper extends HttpServletRequestWrapper {
    ...
    public HttpSession getSession(boolean create) {
      ...
      Boolean allow =
          (Boolean)
              getAttribute(GeoServerSecurityContextPersistenceFilter.ALLOWSESSIONCREATION_ATTR);
      ...
      if (getPathInfo().startsWith("/web") || Boolean.TRUE.equals(allow)) {
        if (LOGGER.isLoggable(Level.FINE)) {
          ...
          e.fillInStackTrace();
          LOGGER.log(
              Level.FINE, "Creating a new http session inside the web UI (normal behavior)", e);
        }
      } else {
