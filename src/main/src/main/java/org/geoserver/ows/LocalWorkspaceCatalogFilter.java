    ...
    if (ResourceInfo.class.isAssignableFrom(clazz)) {
      ...
      if (localPublished != null) {
        if (localPublished instanceof LayerInfo) {
          ...
          filter =
              Predicates.and(
                  filter, factory.equals(factory.literal(Boolean.TRUE), subLayersHidden));
      ...
      if (localPublished == null) {
        ...
        if (request != null
            ...
            && "GetCapabilities".equals(request.getRequest())
            && lg.getMode() == Mode.SINGLE) {
          // wms GetCapabilies with a group in "single" mode, meaning the layers are also showing up
          // stand alone
          // but we only asked for the group, so don't accept any sub-layer
          return Predicates.acceptNone();
        } else {
          // not a WMS capabilities or not a "single" mode layer group, allow any layer in the
          // group,
        ...
        throw new RuntimeException(
            "Unexpected local published reference of type " + localPublished.getClass());
      }
