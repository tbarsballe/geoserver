...
public interface ThreadLocalTransfer {
  ...
  /**
   * Collects the current thread locals values into the storage. The implementation must try hard to
   * use a unique name for each key (a good choice is fullyQualifiedClassName#thredLocalFieldName),
   * the caller utility will make sure there are no conflicts (in case of conflict an exception will
   * be thrown)
   */
