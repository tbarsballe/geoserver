...
public class GeoServerImpl implements GeoServer, ApplicationContextAware {
  ...
  public <T extends ServiceInfo> T getService(Class<T> clazz) {
    ...
    if (service == null) {
      LOGGER.log(
          Level.SEVERE, "Could not locate service of type " + clazz + ", local workspace is " + ws);
    }
  ...
  public void dispose() {
    ...
    for (GeoServerLifecycleHandler handler :
      ...
      try {
        ...
        LOGGER.log(
            Level.SEVERE, "A GeoServer lifecycle handler threw an exception during dispose", t);
      }
    }
  ...
  public void reload(Catalog newCatalog) throws Exception {
    ...
    for (GeoServerLifecycleHandler handler : handlers) {
      try {
        ...
        LOGGER.log(
            Level.SEVERE, "A GeoServer lifecycle handler threw an exception during reload", t);
      }
    }
  ...
  public void reset() {
    ...
    for (GeoServerLifecycleHandler handler :
      ...
      try {
        ...
        LOGGER.log(
            Level.SEVERE, "A GeoServer lifecycle handler threw an exception during reset", t);
      }
    }
  }
}
