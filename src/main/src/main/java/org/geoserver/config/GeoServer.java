...
public interface GeoServer {
   ...
   *
   * <p>This method will return {@link GeoServerInfo#getSettings()} unless a local workspace is set.
   * In that case the settings for that workspace will be checked via {@link
  ...
  /**
   * GeoServer services filtered by class. In the local workspace, or global services if there's no
   * local workspace.
   *
   * <p>
   *
   ...
   *
   * <p>This method should not be called by client code. It is meant to be called internally by the
