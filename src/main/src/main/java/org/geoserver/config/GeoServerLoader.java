...
public abstract class GeoServerLoader {
      ...
      try (AsynchResourceIterator<WorkspaceContents> it =
        ...
        while (it.hasNext()) {
          ...
          try {
            ...
            LOGGER.log(
                Level.WARNING, "Failed to load workspace '" + workspaceResource.name() + "'", e);
            continue;
          }
  ...
  private void loadWmsStore(StoreContents storeContents, CatalogImpl catalog, XStreamPersister xp) {
    ...
    try {
      ...
      LOGGER.info(
          "Loaded wmsstore '" + wms.getName() + "', " + (wms.isEnabled() ? "enabled" : "disabled"));
    } catch (Exception e) {
  ...
  private void loadDataStore(
      StoreContents storeContents, CatalogImpl catalog, XStreamPersister xp, boolean checkStores) {
    final Resource storeResource = storeContents.resource;
    DataStoreInfo ds;
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to load data store '" + storeResource.parent().name() + "'", e);
      return;
    }
      ...
      if (featureTypesDir != null) {
        ...
        for (Resource featureTypeDir : featureTypesDir.list()) {
          ...
          Resource destFeatureTypeDir =
              resourceLoader.get(
                  Paths.path("workspaces", namespace, reader.dataStore(), reader.name()));
    ...
    if (!Resources.exists(f)) {
      ...
      if (Resources.exists(f)) {
        try {
          ...
          LOGGER.log(
              Level.WARNING, "Failed to load global configuration file '" + f.name() + "'", e);
        }
      }
      ...
      if (Resources.exists(f)) {
        try {
          ...
          LOGGER.log(
              Level.WARNING, "Failed to load logging configuration file '" + f.name() + "'", e);
        }
      }
  ...
  void loadServices(
      Resource directory, boolean global, List<XStreamServiceLoader> loaders, GeoServer geoServer) {
