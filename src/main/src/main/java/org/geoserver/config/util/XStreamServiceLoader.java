...
public abstract class XStreamServiceLoader<T extends ServiceInfo> implements ServiceLoader<T> {
  ...
  /**
   * Fills in the blanks of the service object loaded by XStream. This implementation makes sure all
   * collections in {@link ServiceInfoImpl} are initialized, subclasses should override to add more
   * specific initializations (such as the actual supported versions and so on)
   *
   * @param service
   */
