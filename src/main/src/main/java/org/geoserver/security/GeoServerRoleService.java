...
public interface GeoServerRoleService extends GeoServerSecurityService {
  ...
  /**
   * Creates a {@link GeoServerRole} object . Implementations can use their special classes derived
   * from {@link GeoServerRole}
   *
   * @param role
   */
   ...
   * "employeeNumber", which has no value or a default value. "employeeNumber" is also called a role
   * parameter in this context.
   *
   * <p>A user "harry" has assigned the role employee and has a user property "empNr" with the value
   * 4711
   *
  ...
  /**
   * @return the local role having the same privileges as {@link GeoserverRole#ADMIN_ROLE} or <code>
   *     null</code> if no such role exists
   */
