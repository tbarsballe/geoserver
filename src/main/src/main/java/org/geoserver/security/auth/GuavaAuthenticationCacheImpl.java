...
public class GuavaAuthenticationCacheImpl implements AuthenticationCache, DisposableBean {
  ...
  /**
   * Default concurrency level (allows guava cache to optimize internal size to serve the given # of
  ...
  public GuavaAuthenticationCacheImpl(
    ...
    if (LOGGER.isLoggable(Level.INFO)) {
      LOGGER.info(
          "AuthenticationCache Eviction Task created to run every " + cleanUpSeconds + " seconds");
    }
  }
  ...
  public void removeAll(String filterName) {
    ...
    if (LOGGER.isLoggable(Level.FINE)) {
      LOGGER.fine(
          "AuthenticationCache removed " + toBeRemoved.size() + " entries for " + filterName);
      LOGGER.fine("Cache entries #: " + cache.size());
    }
  }
