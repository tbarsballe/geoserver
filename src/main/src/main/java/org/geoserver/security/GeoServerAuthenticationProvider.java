...
public abstract class GeoServerAuthenticationProvider extends AbstractGeoServerSecurityService
  ...
  /**
   * Same function as {@link #authenticate(Authentication)} but is provided with the current request
   * object.
   *
   * <p>This method should never throw an {@link AuthenticationException}. Throwing back such an
   * exception interrupts the authentication procedure in {@link GeoServerSecurityManager} and will
   * prevent providers down the chain from processing the authentication request.
   *
