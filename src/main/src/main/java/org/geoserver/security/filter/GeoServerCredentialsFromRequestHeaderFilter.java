...
public class GeoServerCredentialsFromRequestHeaderFilter extends GeoServerSecurityFilter
  ...
  /**
   * Parse an header string to extract the credential. The regular expression must contain a group,
   * that will represent the credential to be extracted.
   *
  ...
  /**
   * Try to authenticate. If credentials are found in the configured header(s), then authentication
   * is delegated to the AuthenticationProvider chain.
   *
   * @param request
   * @param response
   */
