...
public abstract class AbstractAccessRuleDAO<R extends Comparable<?>> {
  ...
  /**
   * Returns the list of rules contained in the property file. The returned rules are sorted against
   * the <code>R</code> natural order
   */
  ...
  /**
   * Returns the last modification date of the rules in this DAO (last time the rules were reloaded
  ...
  public void storeRules() throws IOException {
    ...
    try {
      ...
      if (e instanceof IOException) throw (IOException) e;
        ...
        throw (IOException)
            new IOException("Could not write rules to " + propertyFileName).initCause(e);
    } finally {
      if (os != null) os.close();
    }
  }
