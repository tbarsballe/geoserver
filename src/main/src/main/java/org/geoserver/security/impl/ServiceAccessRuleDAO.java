...
public class ServiceAccessRuleDAO extends AbstractAccessRuleDAO<ServiceAccessRule> {
  ...
  /**
   * Parses a single layer.properties line into a {@link DataAccessRule}, returns false if the rule
   * is not valid
   */
  ServiceAccessRule parseServiceAccessRule(String ruleKey, String ruleValue) {
    ...
    if (elements.length != 2) {
      LOGGER.warning(
          "Invalid rule " + rule + ", the expected format is service.method=role1,role2,...");
      return null;
    }
