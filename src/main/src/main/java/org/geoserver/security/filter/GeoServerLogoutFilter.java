...
public class GeoServerLogoutFilter extends GeoServerSecurityFilter {
  ...
  /**
   * Search for filters implementing {@link LogoutHandler}. If such a filter is on an active filter
   * chain and is not enlisted in the parameter skipHandlerName, add it to the result
   *
   * <p>The skipHandlerName parameter gives other LogoutHandler the chance to trigger a using {@link
   * #doLogout(HttpServletRequest, HttpServletResponse, String...)} without receiving an unnecessary
   * callback.
   *
