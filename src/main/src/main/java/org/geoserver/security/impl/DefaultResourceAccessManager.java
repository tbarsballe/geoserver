...
public class DefaultResourceAccessManager implements ResourceAccessManager, DataAccessManager {
  ...
  public boolean canAccess(
    ...
    try {
      ...
      LOGGER.log(
          Level.FINE, "Errors occurred trying to gather workspace of resource " + resourceName);
      // it's a layer whose resource we don't know about
      return true;
    }
    ...
    // grab the groups containing the resource, if any. If none, there is no group related logic to
    // apply
    ...
    // there are groups, so there might be more specific rules overriding the catalog one, search
    // for them
    ...
    // the rules allow no access, but there might still be a non secured layer group allowing access
    // to the resource
    return containers
        .stream()
        .anyMatch(
  ...
  /**
   * Returns true if there is a path from the group to the resource that does not involve crossing a
   * opaque group
   */
  ...
  private boolean canAccess(Authentication user, LayerGroupInfo layerGroup, boolean directAccess) {
    ...
    if (node != null && !catalogNodeAllowsAccess) {
      ...
      // grab the groups containing the group, if any. If none, there is no group related logic to
      // apply
      ...
      if (directContainers.isEmpty()) {
        ...
        // do we have at least one path that authorizes access to this group? need to check group by
        // group
        allowAccess =
            directContainers
                .stream()
