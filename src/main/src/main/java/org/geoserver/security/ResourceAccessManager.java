...
public interface ResourceAccessManager {
  ...
  /**
   * Returns a filter selecting only the objects authorized by the manager. May return {@code null}
   * in which case the caller is responsible for building a filter based on calls to the manager's
   * other methods.
   *
   * @param user
   * @param clazz
