...
public class XMLSecurityConfigValidator extends SecurityConfigValidator {
  ...
  /**
   * Additional Validation. Removing this configuration may also remove the file where the roles are
   * contained. (the file may be stored within the configuration sub directory). The design insists
   * on an empty role file.
   */
  @Override
  ...
  /**
   * Additional Validation. Removing this configuration may also remove the file where the users and
   * groups are contained. (the file may be stored within the configuration sub directory). The
   * design insists on an empty user/group file.
   */
  @Override
  public void validateRemoveUserGroupService(SecurityUserGroupServiceConfig config)
    ...
    try {
      ...
      if (file.isAbsolute()
          && !file.getCanonicalPath()
              .startsWith(
                  manager.userGroup().get(config.getName()).file().getCanonicalPath()
