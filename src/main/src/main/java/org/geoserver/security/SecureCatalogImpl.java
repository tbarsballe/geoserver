...
public class SecureCatalogImpl extends AbstractDecorator<Catalog> implements Catalog {
  ...
  /**
   * How to behave in case of mixed mode catalog access, hide the resource or challenge the user to
   * authenticate. For any direct access (by name, id) do challenge, for any "catch all" or "catch
   * group" access, where the single resource was not explicitly requested, hide.
   */
  ...
  public CoverageStoreInfo getCoverageStoreByName(String workspaceName, String name) {
    return checkAccess(
        user(), delegate.getCoverageStoreByName(workspaceName, name), MixedModeBehavior.CHALLENGE);
  }

  public CoverageStoreInfo getCoverageStoreByName(WorkspaceInfo workspace, String name) {
    return checkAccess(
        user(), delegate.getCoverageStoreByName(workspace, name), MixedModeBehavior.CHALLENGE);
  }
  ...
  public DataStoreInfo getDataStoreByName(String workspaceName, String name) {
    return checkAccess(
        user(), delegate.getDataStoreByName(workspaceName, name), MixedModeBehavior.CHALLENGE);
  }
  ...
  public FeatureTypeInfo getFeatureTypeByStore(DataStoreInfo dataStore, String name) {
    return checkAccess(
        user(), delegate.getFeatureTypeByStore(dataStore, name), MixedModeBehavior.CHALLENGE);
  }

  public FeatureTypeInfo getFeatureTypeByDataStore(DataStoreInfo dataStore, String name) {
    return checkAccess(
        user(), delegate.getFeatureTypeByDataStore(dataStore, name), MixedModeBehavior.CHALLENGE);
  }
  ...
  public LayerGroupInfo getLayerGroupByName(String workspaceName, String name) {
    return checkAccess(
        user(), delegate.getLayerGroupByName(workspaceName, name), MixedModeBehavior.CHALLENGE);
  }
  ...
  public <T extends ResourceInfo> T getResourceByStore(
    ...
    return checkAccess(
        user(), delegate.getResourceByStore(store, name, clazz), MixedModeBehavior.CHALLENGE);
  }
  ...
  public <T extends StoreInfo> T getStoreByName(String workspaceName, String name, Class<T> clazz) {
    return checkAccess(
        user(), delegate.getStoreByName(workspaceName, name, clazz), MixedModeBehavior.CHALLENGE);
  }

  public <T extends StoreInfo> T getStoreByName(
      WorkspaceInfo workspace, String name, Class<T> clazz) {
    return checkAccess(
        user(), delegate.getStoreByName(workspace, name, clazz), MixedModeBehavior.CHALLENGE);
  }
  ...
  protected <T extends CatalogInfo> T checkAccess(
    ...
    if (info instanceof LayerGroupInfo) {
      return (T)
          checkAccess(user, (LayerGroupInfo) info, mixedModeBehavior, Collections.emptyList());
    }
  ...
  /**
   * Given a {@link FeatureTypeInfo} and a user, returns it back if the user can access it in write
   * mode, makes it read only if the user can access it in read only mode, returns null otherwise
   */
   ...
   * {@code infoType}.
   *
   * <p>IMPLEMENTATION NOTE: the predicate returned evaluates in-process and hence can't be encoded
   * to the catalog's native query language, if any. It calls {@link
   * #buildWrapperPolicy(Authentication, CatalogInfo)} to check if the returned access level is not
   * "hidden" on a case by case basis. Perhaps, the check for whether a given resource is accessible
   * to the current user can be encoded as a "well known" predicate that uses one or a combination
   * of the property equals/isnull/contains/exists verbs in the {@link Predicates} utility. I (GR),
   * at the time of writing, don't know how to do that, so any help would be much appreciated.
   * Nonetheless, this predicate is meant to be "and'ed" with any other predicate this catalog
   * wrapper is called with, giving the Catalog backend a chance to at least encode the "well known"
   * part of the resulting filter, and separate out the in-process evaluation of access credentials
   * from the construction of the security wrapper for each object.
   *
