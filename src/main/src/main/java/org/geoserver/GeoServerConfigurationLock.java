...
public class GeoServerConfigurationLock {
  ...
  public void lock(LockType type) {
    ...
    if (LOGGER.isLoggable(LEVEL)) {
      LOGGER.log(
          LEVEL, "Thread " + Thread.currentThread().getId() + " got the lock in mode " + type);
    }
  }
  ...
  public boolean tryLock(LockType type) {
    ...
    if (LOGGER.isLoggable(LEVEL)) {
      if (res) {
        ...
        LOGGER.log(
            LEVEL,
            "Thread " + Thread.currentThread().getId() + " could not get the lock in mode " + type);
      }
    }
  ...
  /**
   * Tries to upgrade the current read lock to a write lock. If the current lock is not a read one,
  ...
  public void unlock() {
    ...
    try {
      ...
      if (LOGGER.isLoggable(LEVEL)) {
        LOGGER.log(
            LEVEL,
            "Thread " + Thread.currentThread().getId() + " releasing the lock in mode " + type);
      }
      lock.unlock();
    } finally {
      currentLock.set(null);
