...
public class PartialBufferedOutputStream2 extends OutputStream {
  ...
  /**
   * Closes the stream. If we're still working against the in memory buffer that will be written out
   * before close occurs
   */
   ...
   * real stream. This is why we have a buffer. Abort will succeed if the buffer is not full yet. If
   * that is true, then the buffer is cleared and closed. It does NOT close the response's
   * OutputStream
   *
   * @return DOCUMENT ME!
   * @throws IOException
