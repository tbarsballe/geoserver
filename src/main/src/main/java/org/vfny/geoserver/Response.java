...
public interface Response {
   ...
   * minimal posible risk of failure other than not beeing able to write to the output stream due to
   * external reassons
   *
   * <p>We should clarify when a ServiceException is thrown? I would assume that a "failed" request
   * should still result in a Response that we could write out.
   *
   ...
   *     reader; resulting in a Request object more usefull than a set of raw parameters, as can be
   *     the list of feature types requested as a set of FeatureTypeInfo objects rather than just a
   *     list of String type names
   ...
   *
   * <p>thinked to be called after excecute(), this method must return the MIME type of the response
   * content that will be writen when writeTo were called
   *
   * <p>an implementation of this interface is required to throw an IllegalStateException if execute
   * has not been called yet, to indicate that an inconsistence in the work flow that may result in
   * an inconsistence between the response content and the content type declared for it, if such an
   * implementation can return different contents based on the request that has originated it. i.e.
   * a WMS GetMap response will return different content encodings based on the FORMAT requested, so
   * it would be impossible to it knowing the exact MIME response type if it has not processed the
   * request yet.
   *
  ...
  /**
   * Returns any special content encoding this response will encode its contents to, such as "gzip"
   * or "deflate"
   *
   ...
   * object.
   *
   * <p>In particular, a WMS might wish to have some external caching information added to the HTTP
   * response, so that caches can hang onto this map for a while and ligten the load on geoserver.
   */
   ...
   * @throws IOException ONLY if an error occurs trying to write content to the passed OutputStream.
   *     By this way, we'll can control the very common situation of a java.net.SocketException
   *     product of the client closing the connection (like a user pressing it's refresh browser
   *     button many times)
   */
