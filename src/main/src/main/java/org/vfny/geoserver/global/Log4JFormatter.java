...
public class Log4JFormatter extends Formatter {
  ...
  /**
   * The base logger name. This is used for shortening the logger name when formatting message. For
   * example, if the base logger name is "org.geotools" and a log record come from the
   ...
   *     message. For example, if the base logger name is "org.geotools" and a log record come from
   *     the "org.geotools.core" logger, it will be formatted as "[LEVEL core]" (i.e. the
   ...
   * System#err standard error stream}). This action has no effect on any loggers outside the <code>
   * base</code> namespace.
   *
  ...
  /**
   * Returns the header width. This is the default value to use for {@link #margin}, if no value has
   * been explicitely set. This value can be set in user's preferences.
   *
   ...
   *
   * @task TODO: This class should subclass {@link ConsoleHandler}. Unfortunatly, this is currently
   *     not possible because {@link ConsoleHandler#setOutputStream} close {@link System#err}. If
   *     this bug get fixed, then {@link #close} no longer need to be overriden.
   */
  private static final class Stdout extends StreamHandler {
    ...
    /**
     * Override {@link StreamHandler#close} to do a flush but not to close the output stream. That
     * is, we do <b>not</b> close {@link System#out}.
     */
    public void close() {
      flush();
    }
