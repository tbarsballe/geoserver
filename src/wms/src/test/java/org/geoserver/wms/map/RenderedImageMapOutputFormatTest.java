...
public class RenderedImageMapOutputFormatTest extends WMSTestSupport {
   ...
   * results. This test was originally created after fixes to GEOS-7270 where there were issues with
   * images generated during the direct raster path but not in the normal path, stemming from not
   * setting the background color the same way
   */
  @Test
  public void testDirectVsNonDirectRasterRender() throws Exception {
    ...
    Layer l =
        new CachedGridReaderLayer(reader, builder.createStyle(builder.createRasterSymbolizer()));
    map.addLayer(l);
  ...
  public void testTimeoutOption() throws Exception {
    ...
    LOGGER.info(
        "about to create map ctx for " + map.layers().size() + " layers with bounds " + env);
  ...
  public void testRenderingErrorsHandling() throws Exception {
    ...
    assertNotNull(
        forceRenderingError(new NoninvertibleTransformException("fake non invertible exception")));
    assertNotNull(
        forceRenderingError(new IllegalAttributeException("non illegal attribute exception")));
  ...
  /**
   * Test to check if we can successfully create a direct rendered image by using a coverage view as
   * a source, and a symbolizer defining which three bands of the input coverage view can be used
   * for RGB coloring, and with what order.
   */
  @Test
  public void testStyleUsingChannelsFromCoverageView() throws Exception {
    ...
    final CoverageBand b0 =
        new CoverageBand(
            Collections.singletonList(ib0), "multiband@2", 0, CompositionType.BAND_SELECT);

    final InputCoverageBand ib1 = new InputCoverageBand("multiband", "1");
    final CoverageBand b1 =
        new CoverageBand(
            Collections.singletonList(ib1), "multiband@1", 1, CompositionType.BAND_SELECT);

    final InputCoverageBand ib2 = new InputCoverageBand("multiband", "0");
    final CoverageBand b2 =
        new CoverageBand(
            Collections.singletonList(ib2), "multiband@0", 2, CompositionType.BAND_SELECT);

    final InputCoverageBand ib3 = new InputCoverageBand("multiband", "0");
    final CoverageBand b3 =
        new CoverageBand(
            Collections.singletonList(ib3), "multiband@0", 0, CompositionType.BAND_SELECT);

    final InputCoverageBand ib4 = new InputCoverageBand("multiband", "1");
    final CoverageBand b4 =
        new CoverageBand(
            Collections.singletonList(ib4), "multiband@1", 1, CompositionType.BAND_SELECT);

    final InputCoverageBand ib5 = new InputCoverageBand("multiband", "2");
    final CoverageBand b5 =
        new CoverageBand(
            Collections.singletonList(ib5), "multiband@2", 2, CompositionType.BAND_SELECT);
   ...
   * reading parameter is passed to a coverage reader that does not support it. Reader should ignore
   * it, resulting coverage should not be affected.
   */
  @Test
   ...
   *
   * <p>If the rendering succeeded returns the image, which is going to be a blank one but means the
   * renderer didn't complain about the exception caught. Otherwise throws back the exception thrown
   * by {@link RenderedImageMapOutputFormat#produceMap()}
   */
  ...
  /**
   * Test to make sure the rendering does not skip on unmatched original envelope and tries anyways
   * to render an output
   */
  @Test
