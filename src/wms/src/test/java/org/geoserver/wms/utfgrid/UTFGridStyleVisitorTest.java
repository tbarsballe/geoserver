...
public class UTFGridStyleVisitorTest {
  ...
  public void testOnlyTextSymbolizer() {
    Style style =
        sb.createStyle(sb.createTextSymbolizer(Color.BLACK, sb.createFont("Serif", 12), "name"));
  ...
  public void testFill() {
    ...
    PolygonSymbolizer ls =
        (PolygonSymbolizer) copy.featureTypeStyles().get(0).rules().get(0).symbolizers().get(0);
  ...
  public void testLine() {
    ...
    Style copy = (Style) visitor.getCopy();
    LineSymbolizer ls =
        (LineSymbolizer) copy.featureTypeStyles().get(0).rules().get(0).symbolizers().get(0);
    Stroke stroke = ls.getStroke();
    assertEquals(colorFunction, stroke.getColor());
    assertEquals(Integer.valueOf(3), stroke.getWidth().evaluate(null, Integer.class));
    assertNull(stroke.dashArray());
  ...
  public void testStrokedLine() {
    Style style =
        sb.createStyle(
            sb.createLineSymbolizer(sb.createStroke(Color.BLACK, 1, new float[] {8, 8})));
    style.accept(visitor);
    Style copy = (Style) visitor.getCopy();
    LineSymbolizer ls =
        (LineSymbolizer) copy.featureTypeStyles().get(0).rules().get(0).symbolizers().get(0);
  ...
  public void testRasterToVectorTransform() {
    ...
    Function levels =
        ff.function("parameter", ff.literal("levels"), ff.literal("1100"), ff.literal("1200"));
