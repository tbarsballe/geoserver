...
public class TinyRasterBoundingBoxTest extends WMSTestSupport {
  ...
  public void testTinyRasterBboxContained() throws Exception {
    ...
    Rectangle imageBounds =
        produceMap(
            center.x + offset, center.x + 2 * offset, center.y + offset, center.y + 2 * offset);
    ...
    assertTrue(
        "Expected " + imageBounds + " to contain " + roiBounds, imageBounds.contains(roiBounds));
  }
  ...
  private Rectangle produceMap(double minX, double maxX, double minY, double maxY) {
    this.map
        .getViewport()
        .setBounds(new ReferencedEnvelope(minX, maxX, minY, maxY, DefaultGeographicCRS.WGS84));
