...
public class CapabilitiesIntegrationTest extends WMSTestSupport {
  ...
  public void testFilteredCapabilitiesCite() throws Exception {
    ...
    assertTrue(
        xpath.getMatchingNodes("//wms:Layer/wms:Name[starts-with(., cite)]", dom).getLength() > 0);
    assertEquals(
        0,
        xpath.getMatchingNodes("//wms:Layer/wms:Name[not(starts-with(., cite))]", dom).getLength());
  }
  ...
  public void testLayerQualified() throws Exception {
    ...
    Document dom =
        dom(get("cite/Forests/wms?service=WMS&request=getCapabilities&version=1.3.0"), true);
    // print(dom);
    Element e = dom.getDocumentElement();
    assertEquals("WMS_Capabilities", e.getLocalName());
    XpathEngine xpath = XMLUnit.newXpathEngine();
  ...
  public void testLayerGroup() throws Exception {
    ...
    try {
      ...
      assertXpathEvaluatesTo(
          "2", "count(//wms:Layer[wms:Name='MyLayerGroup']/wms:KeywordList/wms:Keyword)", doc);
      assertXpathEvaluatesTo(
          "keyword1",
  ...
  public void testExceptions() throws Exception {
    ...
    try {
      ...
      doc = getAsDOM("wms?service=WMS&request=getCapabilities&version=1.3.0", true);
      assertXpathEvaluatesTo(
          "JSONP", "wms:WMS_Capabilities/wms:Capability/wms:Exception/wms:Format[5]", doc);
      assertXpathEvaluatesTo(
          "5", "count(wms:WMS_Capabilities/wms:Capability/wms:Exception/wms:Format)", doc);
      JSONType.setJsonpEnabled(false);
      doc = getAsDOM("wms?service=WMS&request=getCapabilities&version=1.3.0", true);
      assertXpathEvaluatesTo(
          "4", "count(wms:WMS_Capabilities/wms:Capability/wms:Exception/wms:Format)", doc);
    } finally {
  ...
  public void testKeywordVocab() throws Exception {
    ...
    String xpath =
        "//wms:Layer[wms:Name='" + linesName + "']/wms:KeywordList/wms:Keyword[@vocabulary='bar']";
  ...
  public void testBoundingBoxCRS84() throws Exception {
    ...
    assertXpathExists(
        "/wms:WMS_Capabilities/wms:Capability/wms:Layer/wms:BoundingBox[@CRS = 'CRS:84']", doc);
    assertXpathExists(
  ...
  public void testMetadataLinks() throws Exception {
    ...
    xpath =
        "//wms:Layer[wms:Name='" + layerName + "']/wms:MetadataURL/wms:OnlineResource/@xlink:type";
    assertXpathEvaluatesTo("simple", xpath, doc);

    xpath =
        "//wms:Layer[wms:Name='" + layerName + "']/wms:MetadataURL/wms:OnlineResource/@xlink:href";
  ...
  public void testGlobalBoundingBoxForLayerGroups() throws Exception {
    ...
    try {
      ...
      LayerInfo layer1 =
          cloneVectorLayerIntoWorkspace(workspace, nameSpace, MockData.BUILDINGS.getLocalPart());
      LayerInfo layer2 =
          cloneVectorLayerIntoWorkspace(workspace, nameSpace, MockData.BRIDGES.getLocalPart());
      LayerGroupInfo layerGroup = createLayerGroup(workspace, "NON_ADVERTISED", layer1, layer2);
      // reduce layer group bounds and store the original bounds which correspond to all layers
      // bounds
      ...
      Document document =
          getAsDOM("NON_ADVERTISED/wms?service=WMS&request=getCapabilities&version=1.3.0", true);
  ...
  public void testLayerGroupsPerWorkspace() throws Exception {
    ...
    try {
      ...
      document =
          getAsDOM(
              "LG_TEST_WORKSPACE_A/wms?service=WMS&request=getCapabilities&version=1.3.0", true);
      assertXpathEvaluatesTo(
          "0",
          "count(//wms:Capability/wms:Layer/wms:Layer[wms:Name='LG_TEST_WORKSPACE_A:LAYER_GROUP_A'])",
          document);
      ...
      document =
          getAsDOM(
              "LG_TEST_WORKSPACE_B/wms?service=WMS&request=getCapabilities&version=1.3.0", true);
      assertXpathEvaluatesTo(
          "0",
          "count(//wms:Capability/wms:Layer/wms:Layer[wms:Name='LG_TEST_WORKSPACE_A:LAYER_GROUP_A'])",
          document);
