...
public class GetFeatureInfoIntegrationTest extends WMSTestSupport {
  ...
  protected void onSetUp(SystemTestData testData) throws Exception {
    ...
    testData.addVectorLayer(
        SQUARES, Collections.EMPTY_MAP, "squares.properties", CapabilitiesTest.class, catalog);
    ...
    testData.addVectorLayer(
        STATES, Collections.emptyMap(), "states.properties", CapabilitiesTest.class, catalog);
  ...
  /**
   * As for section 7.4.3.7, a missing or incorrectly specified pair of I,J parameters shall issue a
   * service exception with {@code InvalidPoint} code.
   */
  @Test
  ...
  /**
   * Check GetFeatureInfo returns an error if the format is not known, instead of returning the text
   * format as in https://osgeo-org.atlassian.net/browse/GEOS-1924
   */
  @Test
  ...
  public void testCoverageGML31() throws Exception {
    ...
    XMLAssert.assertXpathEvaluatesTo(
        "26.0", "//wfs:FeatureCollection/gml:featureMembers/wcs:BlueMarble/wcs:RED_BAND", dom);
    XMLAssert.assertXpathEvaluatesTo(
        "70.0", "//wfs:FeatureCollection/gml:featureMembers/wcs:BlueMarble/wcs:GREEN_BAND", dom);
    XMLAssert.assertXpathEvaluatesTo(
        "126.0", "//wfs:FeatureCollection/gml:featureMembers/wcs:BlueMarble/wcs:BLUE_BAND", dom);
  }
   ...
   * geometry type is not defined so this use case will force the styles rendering machinery to deal
   * with a generic geometry.
   */
  @Test
