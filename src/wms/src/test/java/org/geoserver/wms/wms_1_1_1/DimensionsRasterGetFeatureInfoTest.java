...
public class DimensionsRasterGetFeatureInfoTest extends WMSDimensionsTestSupport {
  ...
  public void testSortTime() throws Exception {
    ...
    // this one should be medium
    assertEquals(
        14.51, getFeatureAt(BASE_URL_ONE + "&sortBy=ingestion D", 36, 31, "sf:watertemp"), EPS);
    // this one hot
    assertEquals(
        19.15, getFeatureAt(BASE_URL_ONE + "&sortBy=ingestion D", 68, 72, "sf:watertemp"), EPS);
  }
  ...
  public void testSortTimeElevation() throws Exception {
    ...
    assertEquals(
        14.51,
        getFeatureAt(BASE_URL_ONE + "&sortBy=ingestion D,elevation", 36, 31, "sf:watertemp"),
        EPS);
    // this one hot
    assertEquals(
        19.15,
        getFeatureAt(BASE_URL_ONE + "&sortBy=ingestion D,elevation", 68, 72, "sf:watertemp"),
        EPS);
  }
