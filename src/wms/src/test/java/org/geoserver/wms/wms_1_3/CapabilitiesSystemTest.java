...
public class CapabilitiesSystemTest extends WMSTestSupport {
   ...
   * if a non supported format is requested, the default {@code text/xml} representation shall be
   * returned. The response Content-Type header shall also be {@code text/xml}
   */
  @Test
  ...
  public void testRequestUpdateSequence() throws Exception {
    ...
    dom =
        getAsDOM(
            "ows?service=WMS&request=GetCapabilities&version=1.3.0&updateSequence=" + updateSeq);
    print(dom);
    assertXpathEvaluatesTo("1.3.0", "/ogc:ServiceExceptionReport/@version", dom);
    assertXpathEvaluatesTo(
        "CurrentUpdateSequence", "/ogc:ServiceExceptionReport/ogc:ServiceException/@code", dom);
    ...
    dom =
        getAsDOM(
            "ows?service=WMS&request=GetCapabilities&version=1.3.0&updateSequence="
                + (currUpdateSeq + 1));
    assertXpathEvaluatesTo(
        "InvalidUpdateSequence", "/ogc:ServiceExceptionReport/ogc:ServiceException/@code", dom);
  }
}
