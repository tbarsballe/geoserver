...
public class LayerGroupWorkspaceTest extends WMSTestSupport {
  ...
  public void prepare() throws Exception {
    ...
    global =
        createLayerGroup(
            cat, "base", "base default", layer(cat, MockData.LAKES), layer(cat, MockData.FORESTS));
    cat.add(global);
    ...
    cite =
        createLayerGroup(
            cat, "base", "cite base", layer(cat, MockData.BRIDGES), layer(cat, MockData.BUILDINGS));
    cite.setWorkspace(cat.getWorkspaceByName("cite"));
    cat.add(cite);

    world =
        createLayerGroup(
            cat, "world", "world", layer(cat, MockData.WORLD), layer(cat, MockData.WORLD));
    cat.add(world);
  }
  ...
  public void testSingleLayerGroupInCapabilities() throws Exception {
    ...
    assertXpathExists(
        "/wms:WMS_Capabilities/wms:Capability/wms:Layer/wms:Layer/wms:Name[text() = 'base']", dom);

    // check it doesn't have children Layers
    assertXpathNotExists(
        "/wms:WMS_Capabilities/wms:Capability/wms:Layer/wms:Layer[Name = 'base']/wms:Layer", dom);
  ...
  public void testSharedLayersInCapabilities() throws Exception {
    ...
    assertXpathExists(
        "/wms:WMS_Capabilities/wms:Capability/wms:Layer/wms:Layer/wms:Name[text() = 'base']", dom);
    assertXpathExists(
        "/wms:WMS_Capabilities/wms:Capability/wms:Layer/wms:Layer/wms:Name[text() = 'base2']", dom);
  ...
  public void testNestedSharedGetMap() throws Exception {
    ...
    Document dom = getAsDOM("wms?request=reflect&layers=nested&format=rss");
    assertXpathExists(
        "rss/channel/title[text() = 'cite:Lakes,cite:Forests,cite:Lakes,cite:Forests']", dom);
  }
