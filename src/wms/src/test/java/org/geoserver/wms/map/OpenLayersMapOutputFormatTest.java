...
public class OpenLayersMapOutputFormatTest extends WMSTestSupport {
  ...
  Pattern lookForEscapedParam =
      Pattern.compile(
          Pattern.quote("\"</script><script>alert('x-scripted');</script><script>\": 'foo'"));
  ...
  /**
   * Test for GEOS-5318: xss vulnerability when a weird parameter is added to the request (something
   * like: %3C%2Fscript% 3E%3Cscript%3Ealert%28%27x-scripted%27%29%3C%2Fscript%3E%3Cscript%3E=foo)
   * the causes js code execution.
   *
   * @throws IOException
   */
  @Test
   ...
   * test uses produceMap and reads the resulting output steam to ensure "yx: true" is returned for
   * EPSG:4326, output is false before fix
   *
   * @throws Exception
   */
  @Test
  ...
  public void testOL3vsOL2() throws Exception {
    ...
    String contentFirefox =
        getResponseContent(
            path + "application/openlayers", firefoxAgent, OpenLayers3MapOutputFormat.MIME_TYPE);
    assertThat(contentFirefox, containsString("openlayers3/ol.js"));

    // generic request on browser not supporting OL3
    String contentIE8 =
        getResponseContent(
            path + "application/openlayers", ie8Agent, OpenLayers2MapOutputFormat.MIME_TYPE);
    assertThat(contentIE8, containsString("OpenLayers.js"));

    // ask explicitly for OL2
    String contentOL2 =
        getResponseContent(
            path + "application/openlayers2", firefoxAgent, OpenLayers2MapOutputFormat.MIME_TYPE);
    assertThat(contentOL2, containsString("OpenLayers.js"));

    // ask explicitly for OL3
    String contentOL3 =
        getResponseContent(
            path + "application/openlayers3", firefoxAgent, OpenLayers3MapOutputFormat.MIME_TYPE);
