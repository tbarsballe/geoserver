...
abstract class DimensionHelper {
    ...
    try {
      ...
      for (String dimName : wl.getDimensions().keySet()) {
        if (TIME.equalsIgnoreCase(dimName)) {
          ...
          LOGGER.log(
              Level.WARNING,
              "Skipping custom dimension " + dimName + " in layer " + layerInfo.getName());
        }
      }
  ...
  /**
   * Writes WMS 1.1.1 conforming dimensions (WMS 1.3 squashed dimensions and extent in the same tag
   * instead)
   *
