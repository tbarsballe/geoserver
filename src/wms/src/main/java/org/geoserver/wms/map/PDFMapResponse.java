...
public class PDFMapResponse extends AbstractMapResponse {
   ...
   * Writes the PDF.
   *
   * <p>NOTE: the document seems to actually be created in memory, and being written down to {@code
   * output} once we call {@link Document#close()}. If there's no other way to do so, it'd be better
   * to actually split out the process into produceMap/write?
   *
  ...
  /**
   * Optimized StyledShapePainter that can optimize painting repeated external graphics by encoding
   * them in a native PDF pattern painter, with no repetition
   */
  private static class PDFStyledPainter extends StyledShapePainter {

    /**
     * Re-creating the patterns is expensive and would increase the overall document size, keep a
     * cache of them instead
     */
