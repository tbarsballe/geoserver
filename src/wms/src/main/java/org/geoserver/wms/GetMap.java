...
public class GetMap {
   ...
   * appropriate output format to a {@link GetMapOutputFormat} appropriate for the required format.
   *
   * <p>Preconditions:
   *
   * <ul>
  ...
  public WebMap run(final GetMapRequest request, WMSMapContent mapContent)
    ...
    if ((numTimes > 1 || singleTimeRange) && isMultivaluedSupported) {
      ...
      for (Object currentTime : times) {
        ...
        map =
            executeInternal(mapContent, request, delegate, Arrays.asList(currentTime), elevations);
      ...
      if (singleElevationRange) {
        List<Object> expandElevationList =
            expandElevationList((NumberRange) elevations.get(0), request, maxAllowedFrames);
        if (expandElevationList.size() == 0) {
          map =
              executeInternal(
                  mapContent, request, delegate, times, Arrays.asList(elevations.get(0)));
        } else {
      ...
      for (Object currentElevation : elevations) {
        maxAnimationTimeHelper.checkTimeout();
        map =
            executeInternal(mapContent, request, delegate, times, Arrays.asList(currentElevation));
  ...
  private void accumulateTimes(TreeSet<Date> result, TreeSet<Object> times, int maxAllowedFrames) {
    for (Object time : times) {
      if (time instanceof Date) {
        ...
        Date rangeMid =
            new Date((range.getMinValue().getTime() + range.getMaxValue().getTime()) / 2);
        result.add(rangeMid);
      }
      ...
      for (MapLayerInfo l : request.getLayers()) {
        ...
        if (l.getType() == MapLayerInfo.TYPE_VECTOR) {
          TreeSet<Object> elevations =
              wms.queryFeatureTypeElevations(l.getFeature(), queryRange, maxAllowedFrames);
          accumulateElevations(result, elevations, maxAllowedFrames);
        } else if (l.getType() == MapLayerInfo.TYPE_RASTER) {
          TreeSet<Object> elevations =
              wms.queryCoverageElevations(l.getCoverage(), queryRange, maxAllowedFrames);
          accumulateElevations(result, elevations, maxAllowedFrames);
        }
      }
    } catch (IOException e) {
  ...
  /**
   * Actually computes the WebMap, either in a single shot, or for a particular time/elevation value
   * should there be a list of them
   *
   * @param request
   * @param mapContent
   * @param delegate
      ...
      if (layerType == MapLayerInfo.TYPE_REMOTE_VECTOR) {
        ...
        try {
          ...
          if (LOGGER.isLoggable(Level.SEVERE)) {
            LOGGER.log(
                Level.SEVERE,
                new StringBuffer("Getting feature source: ").append(exp.getMessage()).toString(),
                exp);
          }
  ...
  private void assertMandatory(GetMapRequest request) throws ServiceException {
    ...
    if (env.isNull() || (env.getWidth() <= 0) || (env.getHeight() <= 0)) {
      throw new ServiceException(
          new StringBuffer("The request bounding box has zero area: ").append(env).toString(),
          "InvalidBBox");
    }
  }
   ...
   * @param layers the layers requested in the GetMap request, where to get the per layer definition
   *     filters from.
   * @return a list of filters, one per layer, resulting of anding the user requested filter and the
   *     layer definition filter
   */
   ...
   *     map format
   * @throws ServiceException if no specialization is configured for the output format specified in
   *     <code>request</code> or if it can't be instantiated or the format is not allowed
   */
