  ...
  public void write(FeatureCollectionType results, GetFeatureInfoRequest request, OutputStream out)
    ...
    try {
      ...
      if (collections.size() == 1) {
        header =
            getTemplate(
                FeatureCollectionDecorator.getName(collections.get(0)), "header.ftl", charSet);
        footer =
            getTemplate(
                FeatureCollectionDecorator.getName(collections.get(0)), "footer.ftl", charSet);
      } else {
      ...
      for (int i = 0; i < collections.size(); i++) {
        ...
        if (fc != null && fc.size() > 0) {
          ...
          if (!(fc.getSchema() instanceof SimpleFeatureType)) {
            ...
            content =
                getTemplate(FeatureCollectionDecorator.getName(fc), "complex_content.ftl", charSet);
          }
          if (content == null) {
