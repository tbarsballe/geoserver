...
public class GetCapabilitiesResponse extends BaseCapabilitiesResponse {
  ...
  /**
   * @param wms needed for {@link WMS#getAvailableExtendedCapabilitiesProviders()} in order to check
   *     of internal DTD elements shall be added to the output document
   */
  ...
  public void write(final Object value, final OutputStream output, final Operation operation)
    ...
    if (internalDTDDeclaration == null) {
      ...
      try {
        ...
        source =
            new SAXSource(
                rawCapsReader, new InputSource(new ByteArrayInputStream(rawCapabilities)));
      } catch (Exception e) {
        throw new ServiceException(e);
      }
   ...
   * the list of direct children of the {@code VendorSpecificCapabilities} element, and each {@link
   * ExtendedCapabilitiesProvider#getVendorSpecificCapabilitiesChildDecls()} is added to the list of
   * internal DTD elements, like in the following example:
   *
   * <pre>
