...
class SVGWriter extends OutputStreamWriter {
  ...
  /**
   * if a reference space has been set, returns a translated Y coordinate wich is inverted based on
   * the height of such a reference space, otherwise just returns <code>y</code>
   */
  ...
  public class AttributesSVGHandler extends SVGFeatureWriterHandler {
     ...
     * @task REVISIT: Once we write directly to out, as we should, this method should be simpler, as
     *     we can just write strings with escapes directly to out, replacing as we iterate of chars
     *     to write them.
     */
  ...
  private abstract class SVGFeatureWriter {
     ...
     *
     * <p>While iterating over the coordinate array passed as parameter, this method performs a kind
     * of very basic path generalization, verifying that the distance between the current coordinate
     * and the last encoded one is greater than the minimun distance expressed by the field <code>
     * minCoordDistance</code> and established by the method {@link #setReferenceSpace(Envelope,
     * float) setReferenceSpace(Envelope, blurFactor)}
     *
     * @param coords
     * @throws IOException
     */
  ...
  private class PointWriter extends SVGFeatureWriter {
    ...
    /**
     * overrides writeBounds for points to do nothing. You can get the position of the point with
     * the x and y attributes of the "use" SVG element written to represent each point
     */
  ...
  /**
   * Writer to handle feature types which contain a Geometry attribute that is actually of the class
   * Geometry. This can occur in heterogeneous data sets.
   *
