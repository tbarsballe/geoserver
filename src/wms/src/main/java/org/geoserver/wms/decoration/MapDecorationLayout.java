...
public class MapDecorationLayout {
  ...
  public static class Block {
     ...
     * anchored at either extreme or centered in the X and Y dimensions, independently, allowing for
     * nine possible Positions.
     */
     ...
     *     MapDecoration
     * @param o a Point whose x- and y-coordinates will be interpreted as x- and y-offsets for the
     *     MapDecoration
     * @return A rectangle that is as close to the desired size and position without exceeding the
     *     container bounds
     */
    ...
    /**
     * A Point whose x- and y-coordinates are interpreted as the x- and y-offsets when rendering the
     * MapDecoration
     */
    final Point offset;
  ...
  /**
   * A container for the blocks in this layout. Blocks contain the positioning information, so this
   * contains all the state for the layout.
   *
   * @see {Block}
   */
  List<Block> blocks;
  ...
  private static MapDecorationLayout fromDocument(MapDecorationLayout dl, Document confFile)
    ...
    for (Element e : (List<Element>) confFile.getRootElement().getChildren("decoration")) {
      ...
      if (pos == null) {
        LOGGER.log(
            Level.WARNING, "Unknown affinity: " + e.getAttributeValue("affinity") + " requested.");
        continue;
      }
      ...
      try {
        ...
        LOGGER.log(
            Level.WARNING, "Couldn't interpret size parameter: " + e.getAttributeValue("size"), e);
      }
      ...
      try {
        ...
        LOGGER.log(
            Level.WARNING, "Couldn't interpret size parameter: " + e.getAttributeValue("offset"));
  ...
  /**
   * Resets the graphics to most of its default values, making each block paint independent of what
   * happened before the decoration layout was called, and from each other
   *
   * @param g2d
   */
