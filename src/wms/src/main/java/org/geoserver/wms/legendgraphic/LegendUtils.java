...
public class LegendUtils {
   ...
   * @return the {@link LegendLayout} specified in the provided {@link GetLegendGraphicRequest} or a
   ...
   *     information.
   * @return the {@link Font} specified in the provided {@link GetLegendGraphicRequest} or a default
   *     {@link Font}.
   */
  public static Font getLabelFont(final GetLegendGraphicRequest req) {
    ...
    if (legendOptions.get("fontSize") != null) {
      try {
        ...
        LOGGER.warning(
            "Error trying to interpret legendOption 'fontSize': " + legendOptions.get("fontSize"));
   ...
   *     GetLegendGraphicRequest#getLegendOptions() legend option}, or the default background color
   *     if no bgcolor were passed.
   */
   ...
   *
   * @param r The rule
   * @param scaleDenominator the scale denominator to check if it is between the rule's scale range.
   *     -1 means that it allways is.
  ...
  public static boolean isWithInScale(final Rule r, final double scaleDenominator) {
    return (scaleDenominator == -1)
        || (((r.getMinScaleDenominator() - BufferedImageLegendGraphicBuilder.TOLERANCE)
                <= scaleDenominator)
            && ((r.getMaxScaleDenominator() + BufferedImageLegendGraphicBuilder.TOLERANCE)
                > scaleDenominator));
  }
   ...
   * character). This allows people to force line breaks in their labels by including the character
   * "\" followed by "n" in their label.
   *
