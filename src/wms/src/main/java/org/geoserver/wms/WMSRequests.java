...
public class WMSRequests {
  ...
  public static String getTiledGetMapUrl(
    ...
    HashMap<String, String> params =
        getGetMapParams(req, layer.getTitle(), layerIndex, layer.getStyle().getName(), bbox, kvp);
   ...
   * asbsolute url, it is simply returned. Otherwise the value is appended to the scheme and host of
   * the supplied <tt>request</tt>.
   *
  ...
  static LinkedHashMap<String, String> getGetMapParams(
    ...
    if (layer != null) {
      ...
      if (req.getRawKvp().get("filter") != null) {
        ...
        List filters =
            KvpUtils.readFlat((String) req.getRawKvp().get("filter"), KvpUtils.OUTER_DELIMETER);
        params.put("filter", (String) filters.get(index));
      } else if (req.getRawKvp().get("cql_filter") != null) {
        // split out the filter we need
        List filters =
            KvpUtils.readFlat((String) req.getRawKvp().get("cql_filter"), KvpUtils.CQL_DELIMITER);
