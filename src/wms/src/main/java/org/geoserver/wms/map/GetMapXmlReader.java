...
public class GetMapXmlReader extends org.geoserver.ows.XmlRequestReader {
   ...
   * the SLD 3. grab the rest of the attribute 4. stuff #3 attributes in the request object 5. stuff
   * the SLD info into the request object 6. return GetMap schema is at
   * http://www.opengeospatial.org/docs/02-017r1.pdf (page 18) NOTE: see handlePostGet() for people
   * who put the SLD in the POST and the parameters in the GET.
   */
  ...
  /**
   * This is the hybrid SLD POST way. Normal is the GetMap - with a built in SLD. The alternate, for
   * stupid people, is the WMS parameters in the GET, and the SLD in the post. This handles that
   * case.
   */
  ...
  /**
   * Performs a check to see if we should use the style from the layer info or from the given set of
  ...
  public void validateSchemaGETMAP(File f, GetMapRequest getMapRequest) throws Exception {
    ...
    try {
      ...
      String msg =
          new StringBuffer("Creating remote GETMAP url: ").append(e.getMessage()).toString();
