...
public class GetMapRequest extends WMSRequest implements Cloneable {
  ...
  /**
   * DJB: spec says SRS is *required*, so if they dont specify one, we should throw an error instead
   * we use "NONE" - which is no-projection. Previous behavior was to the WSG84 lat/long (4326)
   *
  ...
  /**
   * Sets the GetMap request value for the FORMAT parameter, which is the MIME type for the kind of
   * image required.
   */
   ...
   *
   * <p>It is used in conjunction with {@link #getMaxFeatures()} to page through a feature set. This
  ...
  private class OptionalParameters implements Cloneable {
    ...
    /**
     * Temporary hack since finding a good tiling origin would require us to compute the bbox on the
     * fly TODO: remove this once we cache the real bbox of vector layers
     */
     ...
     * http://mapserver.gis.umn.edu/docs/howto/wms_time_support/#time-patterns. Can contain {@link
     * Date} or {@link DateRange} objects.
     */
