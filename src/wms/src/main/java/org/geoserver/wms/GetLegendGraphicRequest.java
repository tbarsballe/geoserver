...
public class GetLegendGraphicRequest extends WMSRequest {
   ...
   * legend graphic.
   *
   * <p>This information is parsed from the GetLegendGraphicRequest and supplemented with layer and
   * style configuration as required. This information is provided as a data structure in order to
   * avoid duplicating logic in GetLegendGraphicKvpReader and BufferedImageLegendGraphicBuild.
   *
   ...
   * GetLegendGraphicRequest} and should not be cached. It represents the state of the system at the
   * time of parsing.
   *
  ...
  public class LegendRequest {
    ...
    /**
     * LegendRequest for a feature type, additional details (title and legend graphic) provided by
     * MapLayerInfo.
     *
     * @param featureType
     */
  ...
  /**
   * The default image format in which to produce a legend graphic. Not really used when performing
   * user requests, since FORMAT is a mandatory parameter, but by now serves as a default for
   * expressing LegendURL layer attribute in GetCapabilities.
   */
  ...
  /**
   * should hold FEATURETYPE parameter value, though not used by now, since GeoServer WMS still does
   * not supports nested layers and layers has only a single feature type. This should change in the
   * future.
   */
   ...
   *       rendering. Anything of the following works: "yes", "true", "1". Anything else means
   *       false.
   ...
   *
   * <p>If the <code>TRANSPARENT</code> parameter is not specified, this property defaults to <code>
   * false</code>.
   *
