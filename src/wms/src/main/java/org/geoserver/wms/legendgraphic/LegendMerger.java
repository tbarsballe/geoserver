...
public class LegendMerger {
  ...
  /**
   * Set of options for legend merging. Used to set all needed options for merging a set of icons in
   * a single place
   *
   * @author mauro.bartolomeoli@geo-solutions.it
   */
  ...
  public static BufferedImage mergeRasterLegends(MergeOptions mergeOptions) {
    ...
    if (layout == LegendLayout.VERTICAL) {
      Column[] columns =
          createColumns(
              nodes, mergeOptions.getColumnHeight(), mergeOptions.getColumns(), null, false);
  ...
  public static BufferedImage mergeLegends(
    ...
    if (layout == LegendLayout.VERTICAL) {
      Column[] columns =
          createColumns(
              nodes, mergeOptions.getColumnHeight(), mergeOptions.getColumns(), req, true);
      finalLegend = buildFinalVLegend(columns, mergeOptions);
    }
