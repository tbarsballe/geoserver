...
public class RasterLayerIdentifier implements LayerIdentifier {
  ...
  public List<FeatureCollection> identify(FeatureInfoRequestParameters params, int maxFeatures)
    ...
    GeneralParameterValue[] parameters =
        wms.getWMSReadParameters(
            getMap, layer, filter, sort, params.getTimes(), params.getElevations(), reader, true);
  ...
  private SimpleFeatureCollection wrapPixelInFeatureCollection(
      GridCoverage2D coverage, double[] pixelValues, Name coverageName) throws SchemaException {
  ...
  /**
   * Convert sample dimension description to a valid XML NCName by replacing invalid characters with
