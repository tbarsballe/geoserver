...
public abstract class NearestMatchFinder {
  ...
  /**
   * Returns an implementation of {@link NearestMatchFinder} optimized for the given resource type,
   * or throws an {@link IllegalArgumentException} in case the resource type is not supported
   *
   * @param info
   * @param dimensionInfo
   * @param dimensionName
  ...
  public static NearestMatchFinder get(
      ResourceInfo info, DimensionInfo dimensionInfo, String dimensionName) throws IOException {
      ...
      if (info instanceof FeatureTypeInfo) {
        ...
        if (reader instanceof StructuredGridCoverage2DReader && ENABLE_STRUCTURED_READER_SUPPORT) {
          ...
          return new StructuredReader(
              structured, dd.getStartAttribute(), dd.getEndAttribute(), acceptableRange, dataType);
        } else if (reader instanceof GridCoverage2DReader) {
          return new Reader(
              (GridCoverage2DReader) reader, acceptableRange, dimensionName, dataType);
        }
      }
  ...
  public NearestMatchFinder(
      String startAttribute, String endAttribute, AcceptableRange acceptableRange, Class dataType) {
   ...
   * "point" type, not a range, even if the domain is made of ranges (the WMS spec seems to indicate
   * a instant value needs to be used for the used time in warning
   *
   * @param value The reference value
   * @return The nearest value, or null if the domain was empty. If the nearest value
   *     matches/overlaps the original value, then the original one is returned instead (this allows
   *     to tell apart no match vs exact match vs nearest match and eventually set the WMS HTTP
   *     warning head)
   * @throws IOException
   */
  ...
  private static class Reader extends NearestMatchFinder {
    ...
    public Object getNearest(Object value) throws IOException {
      ...
      Range rangeFilter =
          this.acceptableRange != null ? this.acceptableRange.getSearchRange(value) : null;
